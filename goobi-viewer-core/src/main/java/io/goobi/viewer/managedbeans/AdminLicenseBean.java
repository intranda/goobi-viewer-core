...
public class AdminLicenseBean implements Serializable {
    ...
    public void init() {
        lazyModelTickets = new TableDataProvider<>(new TableDataSource<AccessTicket>() {
            ...
            public List<AccessTicket> getEntries(int first, int pageSize, final String sortField, final SortOrder sortOrder,
                ...
                try {
                    ...
                    return DataManager.getInstance()
                            .getDao()
                            .getActiveTickets(first, pageSize, useSortField, useSortOrder.asBoolean(), filters);
            ...
            public long getTotalNumberOfRecords(Map<String, String> filters) {
                try {
                    return DataManager.getInstance().getDao().getActiveTicketCount(filters);
                } catch (DAOException e) {
        ...
        lazyModelTickets.setEntriesPerPage(DEFAULT_ROWS_PER_PAGE);
        lazyModelTickets.getFilter("pi_email_title_requestMessage");
    }
    ...
    public String renewTicketAction(AccessTicket ticket) throws DAOException {
        ...
        logger.trace("renewTicketAction: {}", ticket.getId());
    ...
    public String rejectTicketAction(AccessTicket ticket) throws DAOException {
        ...
        logger.trace("rejectTicketAction: {}", ticket.getId());

        if (DataManager.getInstance().getDao().deleteTicket(ticket)) {
    ...
    public String deleteTicketAction(AccessTicket ticket) throws DAOException {
        ...
        logger.trace("deleteTicketAction: {}", ticket.getId());
    ...
    public long getNumRecordsWithAccessCondition(String accessCondition) throws DAOException, IndexUnreachableException, PresentationException {
        long records = countRecordsForAccessConditionQuery(SearchHelper.getQueryForAccessCondition(accessCondition, false));
        if (records == 0) {
            // Alternative query for metadata-only restrictions
            records = countRecordsForAccessConditionQuery("+DOCTYPE:METADATA +" + SolrConstants.ACCESSCONDITION + ":\"" + accessCondition + "\"");
        }
        if (records == 0) {
    ...
    public long getRecordCountForLicenseType(LicenseType licenseType) throws DAOException, IndexUnreachableException, PresentationException {
        ...
        if (licenseType.getRecordCount() == null) {
            licenseType.setRecordCount(countRecordsForAccessConditionQuery(SearchHelper.getQueryForAccessCondition(licenseType.getName(), false)));
            if (licenseType.getRecordCount() == 0) {
                // Alternative query for metadata-only restrictions
                licenseType.setRecordCount(countRecordsForAccessConditionQuery(
                        "+DOCTYPE:METADATA +" + SolrConstants.ACCESSCONDITION + ":\"" + licenseType.getName() + "\""));
            }
            if (licenseType.getRecordCount() == 0) {
                // Alternative query for CMS pages with access conditions
    ...
    /**
     * Counts the records matching the given raw access condition query the same way the search does when the user
     * follows the link generated by {@link #getUrlQueryForAccessCondition(String)}, i.e. aggregated to the top-level
     * record ({@link SearchAggregationType#AGGREGATE_TO_TOPSTRUCT}) and constrained by the standard search suffixes
     * (hidden-record filter, collection blacklist, access privilege filter). Building the final query via
     * {@link SearchHelper#buildFinalQuery(String, boolean, jakarta.servlet.http.HttpServletRequest, SearchAggregationType)}
     * guarantees that the displayed record count matches the number of hits the search link returns.
     *
     * @param query raw access condition query (see {@link SearchHelper#getQueryForAccessCondition(String, boolean)})
     * @return Number of records the corresponding search would return
     * @throws IndexUnreachableException if any.
     * @throws PresentationException if any.
     */
    private static long countRecordsForAccessConditionQuery(String query) throws IndexUnreachableException, PresentationException {
        String finalQuery = SearchHelper.buildFinalQuery(query, false, BeanUtils.getRequest(), SearchAggregationType.AGGREGATE_TO_TOPSTRUCT);
