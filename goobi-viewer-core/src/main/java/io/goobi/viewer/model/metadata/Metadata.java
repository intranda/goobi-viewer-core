...
public class Metadata implements MetadataListElement, Serializable {
    ...
    public Metadata(Metadata orig) {
        ...
        this.values.addAll(orig.values.stream()
                .map(v -> new MetadataValue(v.getIddoc(), v.getMasterValue(), v.getLabel()))
                .toList());
     ...
     * {@inheritDoc}
     *
     * @should return same hash code for equal instances with different keys
     */
    @Override
    public int hashCode() {
        final int prime = 31;
    ...
    static String buildHierarchicalValue(String field, String value, Locale locale, String applicationUrl) {
        ...
        Map<String, String> sortFields = DataManager.getInstance()
                .getConfiguration()
    ...
    public boolean populate(StructElement se, StructElement anchorSe, String ownerIddoc, List<StringPair> sortFields,
        ...
        if (group) {
            if (se.getMetadataFields().get(key) == null && parentMetadata == null) {
                // logger.trace("Skip grouped metadata: {} (key: {})", getLabel(), key); //NOSONAR Debug
