...
/**
 * Asynchronously triggers the ContentServer's source-image cache for the page the user just navigated to, so the OpenSeadragon tile burst that
 * follows finds the master image already decoded and resident in the {@link SourceImageCache}.
 *
 * <p>
 * Without prewarm, the first viewer of a cold-cache page pays the full source-decode latency spread across the parallel tile requests: every tile
 * thread either ROI-decodes the source itself (cache disabled) or waits for the burst-coalesce decoder (cache enabled but cold — the first tile
 * triggers the decode, every subsequent tile waits). With prewarm, the decoder runs once during HTML render — by the time the browser starts
 * requesting tiles, the cache already holds the decoded {@code BufferedImage} and every tile is served via cheap
 * {@link java.awt.image.BufferedImage#getSubimage}.
 *
 * <p>
 * The service is a JVM-wide singleton because the underlying {@link SourceImageCache} is itself a singleton and a single bounded executor avoids one
 * Tomcat session monopolising decode threads at the expense of others. Tasks are submitted to a small pool (4 threads) with a bounded queue; overflow
 * is dropped silently — prewarm is best-effort, never load-bearing.
 *
 * <p>
 * The decode runs on a worker thread that calls {@code getRenderedImage(null)} on a fresh {@link ImageManager}. That call hits
 * {@code AbstractImageInterpreter}'s cache hook which — after the prewarm thread's own {@code registerRequestAndIsBurst} primer — takes the
 * burst-coalesce path and lands in {@link SourceImageCache#getOrDecodeFull}. The first thread (us) wins the race, decodes the master, and populates
 * the cache. Tile threads arriving concurrently see the in-flight {@link java.util.concurrent.CompletableFuture} and wait, which is exactly the
 * desired coalescing behavior.
 */
public final class SourceImagePrewarmService {
    ...
    /**
     * Resolves the source-image URI of the given page and submits an async prewarm. Silently returns when the page is null, has no usable file path,
     * or fails to resolve — prewarm is best-effort and never throws to the caller.
     *
    ...
    /**
     * Resolves the local source-image URI of a page, mirroring the lookup done by {@code ImageHandler#getImageInformation(PhysicalElement)}. Returns
     * null for external URLs (those handled by a remote IIIF server, where prewarming a local cache is meaningless) and for blank file paths.
     */
    ...
    /**
     * Submits an async prewarm for the page identified by record PI and 1-based page order. Returns immediately. The Solr lookup that resolves
     * PI+order to the master file path runs on a worker thread, never on the caller's thread (typically a servlet request thread that must hand the
     * response back to the user as fast as possible).
     *
     * <p>
     * Used by {@code PrewarmRequestFilter} which fires before JSF is even involved — earliest possible point in the request lifecycle to start an
     * async master-image decode.
     *
    ...
    /**
     * Worker-side companion to {@link #prewarmByPiAndPage}. Resolves the master URI via Solr, then runs the decode through the same pipeline as the
     * URI-direct path. Failures are swallowed and counted because prewarm is best-effort.
     */
    ...
    /**
     * Solr lookup: PI + page order → master FILENAME. Returns null when no PAGE doc matches. Runs in the worker thread, never blocks a request
     * thread.
     */
    ...
    /**
     * Submits an async prewarm for the given source-image URI. Returns immediately. The actual decode happens on a worker thread some time later.
     * Safe to call repeatedly with the same URI: a duplicate submission while a previous prewarm is still in flight short-circuits in
     * {@link SourceImageCache#getOrDecodeFull} (one decoder runs, the other waits and serves the same image), and a submission for an already-cached
     * URI returns without doing work.
     *
     * @param sourceUri URI of the master image file (typically {@code file://...}). May be null — null is silently ignored to keep callers free of
     *            preconditions.
    ...
    private void runPrewarm(URI sourceUri) {
        ...
        try {
            ...
            try (ImageManager mgr = new ImageManager(sourceUri)) {
                ...
                // side effect.
                // If the image is too large to be put in cache, use a subsampled version. If the images is still to
                // large for a subsampling rate of 64, throw an error and mark the prewarm attempt as failed
                int subsampling = cache.getMinAllowedSubsampling(mgr.getMyInterpreter().getOriginalImageWidth(),
                        mgr.getMyInterpreter().getOriginalImageHeight());
                mgr.getMyInterpreter().getRenderedImage(null, subsampling);
            }
    ...
    /**
     * Stops the prewarm executor cleanly so Tomcat does not report a memory-leak warning at webapp undeploy. Discards any queued tasks (prewarm is
     * best-effort, so dropping pending decodes is fine), interrupts in-flight worker threads and waits up to 5 seconds for them to terminate.
     *
     * <p>
     * Called from {@code ContextListener#contextDestroyed} BEFORE the Solr client is closed, because {@link #resolveAndPrewarm} performs a Solr
     * lookup; shutting down the executor first guarantees no worker is still running when Solr disappears.
     */
