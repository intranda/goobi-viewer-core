...
var viewerJS = (function (viewer) {
        ...
        this.locations = L.geoJSON([], {
            ...
            onEachFeature: function (feature, layer) {
                ...
                rxjs.fromEvent(layer, 'dragend')
                    .pipe(
                        rxjs.operators.map(() => this.openPopup(layer)),
                        rxjs.operators.map(() => this.updatePosition(layer))
                    )
                    .subscribe(this.onFeatureMove);
                rxjs.fromEvent(layer, 'click')
                    .pipe(rxjs.operators.map((e) => layer.feature))
                    .subscribe(this.onFeatureClick);
    ...
    viewer.GeoMap.featureGroup.prototype.getClusterCount = function (cluster) {
        let count = cluster
            .getAllChildMarkers()
            .map((child) => this.getCount(child.feature.properties))
            .reduce((a, b) => a + b, 0);
        return count;
    };
    ...
    viewer.GeoMap.featureGroup.prototype.updatePosition = function (marker) {
        ...
        marker.feature.view = {
            zoom: this.geoMap.map.getZoom(),
            center: [marker.getLatLng().lng, marker.getLatLng().lat],
        };
    ...
    viewer.GeoMap.featureGroup.prototype.getArea = function (bounds, feature) {
        if (!bounds || !bounds.isValid()) {
            ...
            let area =
                this.geoMap.map.distance(bounds.getSouthWest(), bounds.getNorthWest()) *
                this.geoMap.map.distance(bounds.getSouthWest(), bounds.getSouthEast());
        }
    };
    ...
    viewer.GeoMap.featureGroup.prototype.showMarkers = function (entityFilter) {
        ...
        this.hideMarkers();
        this.markers
            .filter((m) => this.getCount(m.feature.properties))
            .forEach((m) => {
                if (m.setIcon) {
