...
// toggle next cms right block after radio button
	...
	// hide user terms text if radio = no
	// check if toggle yes on page load
if ($('.userTermsToggle input:nth-of-type(2)').prop('checked')) {
		  $('.admin__user-terms-box-editable').hide();
	  }
	// check if toggle status changes
$(".userTermsToggle input").change(function(){
	  if ($('.userTermsToggle input:nth-of-type(2)').is(':checked'))
	   $('.admin__user-terms-box-editable').animate({
		    height: "toggle",
		    opacity: "toggle"
		}, 250);
	  else if ($('.userTermsToggle input:nth-of-type(1)').is(':checked'))
		   $('.admin__user-terms-box-editable').animate({
			    height: "toggle",
			    opacity: "toggle"
			}, 250);
