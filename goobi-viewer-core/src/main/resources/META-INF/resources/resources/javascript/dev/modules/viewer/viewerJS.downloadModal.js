...
var viewerJS = (function (viewer) {
  ...
  var _defaults = {
    ...
    messages: {
      ...
      rcInvalid:
        "Die Überprüfung war nicht erfolgreich. Bitte bestätigen Sie die reCAPTCHA Anfrage.",
      rcValid:
        "Vielen Dank. Sie können nun ihre ausgewählte Datei generieren lassen.",
      eMailText:
  ...
  viewer.downloadModal = {
    ...
    renderModalBody: function (type, infos) {
      if (_debug) {
        console.log(
          "---------- viewer.downloadModal.renderModalBody() ----------"
        );
      ...
      if (infos.size) {
        modalBody +=
          "<dt>" + _defaults.messages.downloadInfo.fileSize + ":</dt>";
      ...
      modalBody +=
        '<label for="recallEMail">' + _defaults.messages.eMail + "</label>";
      if (_defaults.userEmail != undefined) {
        modalBody +=
          '<p class="help-block">' +
          _defaults.messages.eMailTextLoggedIn +
          "</p>";
        modalBody +=
        ...
        modalBody +=
          '<p class="help-block">' + _defaults.messages.eMailText + ":</p>";
        modalBody +=
          '<input type="email" class="form-control" id="recallEMail" />';
      }
      modalBody += "</div>";
      modalBody += "</form>";
    ...
    validateReCaptcha: function (response) {
      if (_debug) {
        console.log(
          "---------- viewer.downloadModal.validateReCaptcha() ----------"
        );
        console.log(
          "viewer.downloadModal.validateReCaptcha: response = ",
          response
        );
      }
      if (response == 0) {
    ...
    buildAPICall: function (path, type, pi, logid, email) {
      if (_debug) {
        console.log(
          "---------- viewer.downloadModal.buildAPICall() ----------"
        );
