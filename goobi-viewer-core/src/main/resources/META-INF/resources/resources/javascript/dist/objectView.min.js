...
THREE.FBXMeshLoader.prototype = {
	...
	load : function(url, onLoad, onProgress, onError) {
		...
		$.getJSON(url).then(function(info) {
...
THREE.GLTFDRACOLoader.prototype = {
	...
	load : function(url, onLoad, onProgress, onError) {
		...
		dracoLoader.setDecoderPath( this.manager.worldConfig.resourcesPath + 'three/dependencies/draco/' );
		...
		if(url.endsWith("gltf") || url.endsWith("glb")) {
		    console.log("loading object directly from ", url);
		    ...
		    $.getJSON(url).then(function(info) {
		        console.log("loading object info = ", info);
		        var objUrl = info.uri; 
		        console.log("load from ", objUrl, gltfLoader);
...
THREE.OBJMTLLoader.prototype = {
	...
	load : function(url, onLoad, onProgress, onError) {
		...
		$.getJSON(url).then(function(info) {
			console.log("loading object from info json ", info);
			var baseResourceUrl = info.uri.substring(0, info.uri.lastIndexOf("/"));
			var objUrl = info.uri;
			...
			if(mtlUrl) {	
			    console.log("load textures from ", mtlUrl)
			    textureLoader.parse(mtlUrl.substring(0, mtlUrl));
	...
	load : function(url, onLoad, onProgress, onError) {
		...
		$.getJSON(url).then(function(info) {
			...
			if(info.resources && info.resources.length > 0) {
			    material = new THREE.MeshNormalMaterial({map: textureLoader.load(info.resources[0])});
			} else {
...
X3DLoader.prototype = {
    ...
    load : function($image, url, onLoad, onProgress, onError) {
        ...
        return $.getJSON(url).then(function(info) {
...
var WorldGenerator = (function() {
	...
	var _getObjectLoader = function(config, manager) {
		...
		default:
			console.log("no loader defined for suffix" + suffix);
		}
	}
    ...
    class World {
		constructor(config) {
			...
			this.loadingMonitor = new rxjs.Subject();
			// CAMERA//
			...
			this.renderer.gammaOutput = config.light.gamma.use;
			this.renderer.gammaFactor = config.light.gamma.factor;
			...
			this.loadingManager.worldConfig = this.config;
...
// world.rotateLights(90, 'z');
		...
		/**
         * config.size: diameter of sphere config.material.color: color of the sphere config.material.opacity: if defined, opacity of sphere config.offset: Vector3D defining the
         * offset of the sphere's center from point 0
         */
		addSphere(config) {
		    ...
		    return new Promise( (resolve, reject) => {
				...
				if(loader) {
					...
					loader.load(config.url, function(object) {
						...
						resolve(object);
					}, p => this.loadingMonitor.next({url: p.originalTarget.responseURL, loaded: p.loaded}), function(error) {
						reject(error);
					});
				}
			});
		...
		addObject(object, config) {
			...
			console.log("add object ", object, config);
