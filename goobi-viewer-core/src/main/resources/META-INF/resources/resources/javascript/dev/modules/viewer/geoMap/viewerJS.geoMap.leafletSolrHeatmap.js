	...
	*/
    ...
    L.SolrHeatmapQueryAdapters = {
        ...
        goobiViewer: L.SolrHeatmapBaseQueryAdapter.extend({
            ...
            _solrQuery: function (bounds) {
                ...
                let query =
                    this.layer._heatmapUrl.replace('{solrField}', this.options.field) +
                    '?' +
                    'region=' +
                    encodeURIComponent(region) +
                    '&' +
                    'query=' +
                    this.options.filterQuery;
            ...
            _searchHitsSolrQuery: function (bounds) {
                ...
                let query =
                    this.layer.featureUrl.replace('{solrField}', this.options.field) +
                    '?' +
                    'region=' +
                    encodeURIComponent(region) +
                    '&' +
            ...
            _getBoundsForQuery(bounds) {
                if (bounds === undefined) {
                    ...
                    bounds = new L.latLngBounds(
                        L.latLng(rawBounds.getNorth(), west),
                        L.latLng(rawBounds.getSouth(), east)
                    );
                }
                return bounds;
            },
    ...
    L.SolrHeatmap = L.GeoJSON.extend({
        ...
        _createHeatmap: function () {
            var _this = this;
            var heatmapCells = [];
            var cellSize = _this._getCellSize() * 0.75;
            var colors = _this.options.colors;
            ...
            var options = { max: 0.0001, radius: cellSize, gradient: gradient };
        ...
        _getGradient: function (classifications) {
            ...
            for (var i = 1; i < classifications.length; i++) gradient[classifications[i] / maxValue] = colors[i];
            return gradient;
        },
        ...
        _expandMarker(marker) {
            ...
            if (visibleOne === marker) {
                ...
                if (bounds && this.queryAdapter.ajaxOptionsForSearchHits) {
                    $.ajax(this.queryAdapter.ajaxOptionsForSearchHits(bounds)).then((res) => {
                        res.forEach((geoJson) => {
        ...
        _computeTotalChildHits(cluster) {
            ...
            count += cluster
                .getAllChildMarkers()
                .map((child) => (child.options.count ? child.options.count : 0))
                .reduce((a, b) => a + b, 0);
