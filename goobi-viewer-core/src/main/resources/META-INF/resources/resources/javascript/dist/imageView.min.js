            ...
            if( image.observables ) {
                if(image.sequence) {
                    image.observables.firstTileLoaded.subscribe(e => {
                        ...
                        console.log("minzoom level ", image.viewer.viewport.getMinZoom())
                        console.log("maxzoom level ", image.viewer.viewport.getMaxZoom())
                        this.maxZoomLevel = image.viewer.viewport.getMaxZoom() * image.viewer.viewport.getMinZoom();

                        this.baseYOffset = this.image.viewer.viewport.getBounds(false).y;
                        console.log("base offset ", this.baseYOffset);
...
//                            controls.setPanning(false);
        ...
        imageView.Controls.prototype.goHome = function( immediate ) {
            ...
            if(this.image.sequence) {
                let imageIndex = this.image.sequence.getCurrentImageIndex();
                let imageBounds = this.image.viewer.world.getItemAt(imageIndex)?.getBounds();
                console.log("current image ", imageIndex, imageBounds);
                if(imageBounds) {
                    if(this.baseYOffset) {
...
//    var _fadeout = null;
    ...
    imageView.Sequence = function(config, image) {
            ...
            this.image = image;
            this.locations = [];
            this.currentImage = config.image.currentImageIndex;
            ...
            this.image.observables.viewerOpen.subscribe(openEvent => {
                ...
                this.image.viewer.element.addEventListener("wheel", wheelEvent => {
                    ...
                    } else if(wheelEvent.deltaY < 0 && this.image.viewer.viewport.getBounds().y < 0) {
            ...
            this.getCurrentImagePosition = function() {
                return this.locations[this.currentImage];
            }
            ...
            this.getImageAtPosition = function(y) {
                ...
                for (let index = 0; index < this.locations.length; index++) {
                    if(y < this.locations[index] + this.offset) {
                        return image;
                    } else {
                        image = index;
                    }
            ...
            this.initImageLocations = function(image) {
                ...
                for (let index = 0; index < image.viewer.world.getItemCount(); index++) {
                    ...
                    this.locations.push(item.getBounds().y);
                }
