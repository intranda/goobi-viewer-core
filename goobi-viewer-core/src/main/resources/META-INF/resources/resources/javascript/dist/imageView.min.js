...
//         this.imageViewHeight = parseFloat($('#'+this.config.div).css("height"));
     ...
     imageView.Image.prototype.load = function() {
         ...
         imageView.Image.prototype.loadImage = function(tileSources) {
             ...
             this.observables.viewerOpen.pipe(rxjs.operators.merge(this.observables.redrawRequired))
...
//         }
     ...
     function _createObservables(window, image) {
         var observables = {};
         observables.viewerOpen = rxjs.Observable.create(function(observer) {
         ...
         observables.firstTileLoaded = rxjs.Observable.create(function(observer) {
         ...
         observables.viewerZoom = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'zoom', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.animationComplete = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'animation-finish', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.viewportUpdate = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'update-viewport', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.animation = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'animation', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.viewerRotate = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'rotate', function( event ) {
                 event.osState = "rotate";
                 return observer.next(event);
             } );
         });
         observables.canvasResize = rxjs.Observable.create(function(observer) {
         ...
         observables.windowResize = rxjs.fromEvent(window, "resize").pipe(rxjs.operators.map(function(event) {
             event.osState = "window resize";
             
             return event;
         }));

         observables.overlayRemove = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'remove-overlay', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.overlayUpdate = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'update-overlay', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.levelUpdate = rxjs.Observable.create(function(observer) {
             image.viewer.addHandler( 'update-level', function( event ) {
                 return observer.next(event);
             } );
         });
         observables.close = rxjs.Observable.create(function(observer) {
         ...
         observables.redrawRequired = 
                  observables.viewerRotate.pipe(
                     rxjs.operators.merge(observables.canvasResize.pipe(rxjs.operators.skipUntil(observables.firstTileLoaded), rxjs.operators.catchError(error => rxjs.of(error)))),
...
//                imageView.controls.persistence.init(config);
            ...
            if( image.observables ) {
                ...
                image.observables.redrawRequired.pipe(
                    rxjs.operators.merge(image.observables.viewerOpen),
                    rxjs.operators.sample(image.observables.viewportUpdate))
                .subscribe(function(event) {
...
var ImageView = ( function(imageView) {
    ...
    imageView.DataPoint.Transform = function(viewer, startCondition) {
        ...
        this.drawArea = null;
        this.dataPoints = [];
        this.finishedObservable = new rxjs.Subject();
...
var ImageView = ( function( imageView ) {
    ...
    imageView.Tools.Filter = function(image, filterMethod) {
        ...
        this.subscription = this.image.observables.viewportUpdate.subscribe((event) => {
            let context = image.getCanvasContext();
            let data = context.getImageData(0, 0, context.canvas.width, context.canvas.height);
            let pixels = this.filterMethod(data.data);
...
var ImageView = ( function( imageView ) {
    ...
    imageView.Tools.AreaSelect = function(image, config) {
        ...
        this.drawer = new ImageView.Draw(image.viewer, this.config.drawStyle, () => this.drawing);
        this.drawer.finishedDrawing().subscribe(function(rect) {
            ...
            if(this.finishedHook) {
                ...
                this.finishedHook.resolve(imageArea);
                this.finishedHook = null;
            }
        }.bind(this))
    }
    ...
    imageView.Tools.AreaSelect.prototype.startSelect = function() {
        if(this.finishedHook) {
            this.finishedHook.reject();
        }
        this.drawing = true;
        this.finishedHook = Q.defer();
        return this.finishedHook.promise;
    }
...
var ImageView = ( function( imageView ) { 
    ...
    imageView.Tools.Filter.Brightness.prototype.filter = function(pixels) {
        ...
        for(var i = 0; i < pixels.length; i += 4) {
            var pixelData = [pixels[i], pixels[i+1], pixels[i+2], pixels[i+3]];
            pixels[i] = pixelData[0] + adjustment;
            pixels[i+1] = pixelData[1] + adjustment;
            pixels[i+2] = pixelData[2] + adjustment;
            pixels[i+3] = pixelData[3] + adjustment;
        }
    }
