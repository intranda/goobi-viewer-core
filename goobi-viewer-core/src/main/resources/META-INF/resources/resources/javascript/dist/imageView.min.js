    ...
    imageView.Overlay.prototype.draw = function() {
        ...
        if(this.label) {
            ...
            if(!this.hidden) {
                ...
                console.log("label ", this.label, $svg);
                this.viewer.addOverlay({
                    element: this.label,
                    location: rect,                
                });                
...
var ImageView = ( function( imageView ) { 
    ...
    imageView.Tools.Filter.HighlightArea.prototype.filter = function(pixels) {
        ...
        let rotated = rect%180 === 90;
        for(var i = 0; i < data.length; i += 4) {
            let p = i/4;
            let x = p % pixels.width;
            let y = Math.floor(p/pixels.width);
            if ( rotated &&  x >= rect.x && x < rect.x+rect.height-1 && y >= rect.y && y < rect.y+rect.width-1 ) {
                continue;
            } else if ( x >= rect.x && x < rect.x+rect.width-1 && y >= rect.y && y < rect.y+rect.height ) {
                continue;
            } else {                
                var pixelData = [data[i], data[i+1], data[i+2], data[i+3]];
