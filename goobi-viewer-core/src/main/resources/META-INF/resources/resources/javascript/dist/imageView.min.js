...
//         this.imageViewHeight = parseFloat($('#'+this.config.div).css("height"));
     ...
     imageView.Image.prototype.load = function() {
             ...
             return Promise.all(promises).then(function(tileSources) {
                 ...
                 .then((footerLoaded) => {
                     if(_debug) {                         
                         console.log("loaded footer 1", footerLoaded);
                     }
                    //  if(!footerLoaded) {
         ...
         imageView.Image.prototype.loadImage = function(tileSources) {
			...
			// TEMP
				  window.osviewer = this.viewer;
				 if(_debug) { 
				 	console.log('init OpenSeadragon with osConfig');
				 }
...
var ImageView = ( function(imageView) {
    ...
    function _onViewerDrag( event, transform ) {
        if ( transform.isTransforming() ) {
            ...
            const dx = newPoint.x - transform.lastPoint.x;
            const dy = newPoint.y - transform.lastPoint.y;
            transform.lastPoint = newPoint; 

            if ( transform.drawArea === imageView.Overlay.HitAreas.TOPLEFT ) {

                topLeft = new OpenSeadragon.Point( Math.min( rect.getTopLeft().x + dx, rect.getBottomRight().x ), Math.min( rect.getTopLeft().y + dy, rect.getBottomRight().y ) );
                bottomRight = rect.getBottomRight();
            }
            else if ( transform.drawArea === imageView.Overlay.HitAreas.TOPRIGHT ) {
                topLeft = new OpenSeadragon.Point( rect.getTopLeft().x, Math.min( rect.getTopLeft().y + dy, rect.getBottomRight().y ) );
                bottomRight = new OpenSeadragon.Point( Math.max( rect.getBottomRight().x + dx, rect.getTopLeft().x ), rect.getBottomRight().y );
            }
            else if ( transform.drawArea === imageView.Overlay.HitAreas.BOTTOMLEFT ) {
                topLeft = new OpenSeadragon.Point( Math.min( rect.getTopLeft().x + dx, rect.getBottomRight().x ), rect.getTopLeft().y );
                bottomRight = new OpenSeadragon.Point( rect.getBottomRight().x, Math.max( rect.getBottomRight().y + dy, rect.getTopLeft().y ) );
            }
            else if ( transform.drawArea === imageView.Overlay.HitAreas.BOTTOMRIGHT ) {
                topLeft = rect.getTopLeft();
                bottomRight = new OpenSeadragon.Point( Math.max(  rect.getBottomRight().x + dx, rect.getTopLeft().x ), Math.max(  rect.getBottomRight().y + dy, rect.getTopLeft().y ) );
            }
