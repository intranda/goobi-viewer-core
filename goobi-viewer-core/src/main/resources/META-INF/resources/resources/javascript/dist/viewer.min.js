...
var viewerJS = (function (viewer) {
    ...
    function _hideEmptyPanels() {
        document.querySelectorAll('.fullscreen__view-sidebar-accordeon-panel').forEach((panel) => {
            let childCount = [...panel.querySelectorAll('.fullscreen__view-sidebar-accordeon-panel-body')]
                ...
                if ($('.fullscreen__view-image-thumbs-wrapper').is(':visible')) {
                    setTimeout(function () {
                        $('.fullscreen__view-image-thumbs-wrapper').outerWidth(
                            $('#fullscreenView').outerWidth() - $('#fullscreenViewSidebar').outerWidth()
                        );
                    }, 325);
                }
            },
            ...
            .then((locations) => {
                ...
                if (locations.length > 0) {
                    ...
                    this.markers = this.markers.concat(
                        _addMarkers(this.map, locations, this.config.msg ? this.config.msg.propertiesLink : undefined)
                    );
                }
...
var viewerJS = (function (viewer) {
    ...
    function _initNerPopover(path) {
        ...
        $('.ner-trigger').on('click', function () {
            ...
            textBox.append(
                _renderNerPopover(data, _calculateNerPopoverPosition(triggerCoords, textBoxCoords), title, path)
            );
...
var viewerJS = (function (viewer) {
    ...
    viewer.searchList = {
        ...
        init: function (config) {
            ...
            function bindModalTriggerState(triggerSelector, modalSelector) {
                ...
                function setTriggerState(isOpen) {
                    ...
                    $triggers.toggleClass('is-modal-trigger-open', isOpen).attr('aria-pressed', isOpen);
        ...
        initSubHits: function () {
            document.querySelectorAll('[data-toggle="hit-content"]').forEach((button) => {
...
var viewerJS = (function (viewer) {
    ...
    // Use image URI from data-viewer-access-denied-url if available, otherwise default
    ...
    viewer.GeoMapCms = function (config) {
        ...
        if (this.config.pagesToShow) {
            this.config.map.layers
                .map((layer) => layer.features)
                .flat()
                .filter((f) => !this.config.pagesToShow.includes(f.properties.page) && f.properties.page !== undefined)
...
var cmsJS = (function (cms) {
    ...
    cms.GeoMapEditor = function (config) {
        ...
        this.onMetadataUpdate
            .pipe(
                rxjs.operators.takeWhile(() => this.config.allowEditFeatures),
                rxjs.operators.map((metadata) => this.updateCurrentFeatureMetadata(metadata))
            )
            .subscribe(() => this.saveFeatures());
    ...
    cms.GeoMapEditor.prototype.updateView = function (view) {
        if (!view) {
            ...
            if (this.activeFeatureGroup?.getFeatures()?.length > 0) {
                this.geoMap.setView(
                    this.geoMap.getViewAroundFeatures(this.activeFeatureGroup.getFeatures(), view.zoom)
                );
            } else {
                ...
                switch (action) {
                    ...
                    case 'delete':
                        viewerJS.notifications
                            .confirm('', undefined, undefined, _defaults.msg.bulkDeleteConfirm)
                            .then(() => {
    ...
    cms.modules = {
        ...
        setValidationStatus: function (source) {
            ...
            if (source.indexOf(':') > -1) {
                ...
                status = $('[id*="' + stripedSource + '"]')
                    ...
                    .find('.cms-module__option-message-status, .input_form__option-message-status');
            } else {
            ...
            if (status.hasClass('success')) {
                $('[id*="' + stripedSource + '"]')
                    .parents('.cms-module__option-control, .input_form__option_control')
                    .find('.cms-module__option-message-mark, .input_form__option-message-mark')
                    .addClass('success');
        ...
        initEventListeners: function () {
            ...
            $('body')
                ...
                .on('click.toggle-items', '[data-toggle="available-items"]', function () {
                    if ($(this).hasClass('in')) {
                        $(this).toggleClass('in');
                        $(this)
                            .parent()
            ...
            $('body')
                ...
                .on('click.option-dropdown', '[data-toggle="option-dropdown"]', function () {
                    $(this).next().slideToggle('fast');
                });
...
var cmsJS = (function (cms) {
    ...
    function _handleBeforeDropFromVisible(event, ui) {
        ...
        if (
            ...
            item.parent().attr('id') === $(_config.sortablesConfig.availableItemList).attr('id')
        ) {
            ui.item.remove();
        }
    }
...
var Crowdsourcing = (function (crowdsourcing) {
        ...
        switch (this.targetSelector) {
            case Crowdsourcing.Question.Selector.RECTANGLE:
                if (this.areaSelector) {
                    ...
                    this.annotations.forEach((anno) => this.areaSelector.addOverlay(anno, this.item.image));
    ...
    crowdsourcing.Question.prototype.getAnnotationsFromLocalStorage = function () {
        let targetId =
            this.targetSelector == Crowdsourcing.Question.Selector.WHOLE_SOURCE ? undefined : this.getTargetId();
