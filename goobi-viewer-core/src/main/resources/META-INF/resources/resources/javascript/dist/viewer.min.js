...
//                $dropdown.on("click", (event) => event.stopPropagation());
            ...
            calcDropDownPos: function(dropdown) {
              var dropDownRect = dropdown.getBoundingClientRect();
              var overflowRight = 0;

              // check if dropdown currently has a width
              if (dropDownRect.width === 0) {
                // if not: make it appear for js (it will remain visually hidden)
                dropdown.style.display = 'block';
                // update size + relative pos
                ...
                // return value to handle overflow left
                if(dropDownRect.x < 0) 
                  return (- dropDownRect.x);

                // return value to handel overflow right
                if (overflowRight > 0) 
            ...
            renderSessionBookmarksDropdown: function() {
                ...
                $button.on("click", (event) => {
                    ...
                    var translateVal = this.calcDropDownPos($dropdown[0]);

                    // show/hide dropdown + set it's position
                    $(event.currentTarget).next('.bookmark-navigation__dropdown')
                      .fadeToggle('fast')
                      .css("transform", "translateX(" + translateVal + "px)")
                })
