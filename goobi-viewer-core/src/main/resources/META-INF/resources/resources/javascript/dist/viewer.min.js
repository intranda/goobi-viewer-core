...
var viewerJS = (function () {
  ...
  viewer.init = function (config) {
    ...
    this.pageScroll.init(
      _defaults.pageScrollSelector,
      _defaults.pageScrollAnchor
    );
    ...
    $("body").on(
      "keyup",
      '#pfAutocomplete_input, [id*=":pfAutocomplete_input"]',
      function (event) {
        if (event.keyCode == 13) {
    ...
    $(".widget-chronology-slider__item-input")
      .focus(function () {
        $(
          '.widget-chronology-slider__item-input[data-toggle="tooltip"]'
        ).tooltip("disable");
      })
      .blur(function () {
        $(
  ...
  viewer.showLoader = function () {
    viewer.jsfAjax.complete
      .pipe(rxjs.operators.first())
      .subscribe(() => $(".ajax_loader").hide());
    $(".ajax_loader").show();
  };
        ...
        case "#feedback":
          ...
          $("#feedbackModal").on("hidden.bs.modal", function (e) {
            history.replaceState(
              {},
              "",
              window.location.href.replace(/#feedback.*$/, "")
            );
          });
          break;
        case "#?page":
  ...
  viewer.initRequiredInputs = function () {
    ...
    $requireElements.each((index, element) => {
      ...
      let $uncheckedRadios = $(
        'input[data-require-input-checkbox="uncheckedChecker"]'
      );
...
var viewerJS = (function (viewer) {
  ...
  viewer.helper = {
    ...
    renderAlert: function (type, content, dismissable) {
      ...
      bsAlert +=
        ...
        type +
        ' alert-dismissible fade in show">';
      if (dismissable) {
        bsAlert +=
  ...
  viewer.getMapBoxToken = function () {
    if (
      typeof this.mapBoxConfig != "undefined" &&
      typeof this.mapBoxConfig.token != "undefined"
    ) {
  ...
  viewer.helper.ArrayComparator = function (
    array,
    includeUnsortedBefore,
    valueFunction
  ) {
    this.comparatorArray = array;
    this.notIncludedOrdering =
      includeUnsortedBefore == true
        ? 1
        : includeUnsortedBefore == false
        ? -1
        : 0;
    ...
    this.compare = (a, b) => {
      ...
      if (
        this.comparatorArray.includes(valA) &&
        this.comparatorArray.includes(valB)
      ) {
        return (
          this.comparatorArray.indexOf(valA) -
          this.comparatorArray.indexOf(valB)
        );
    ...
    init: function (config) {
      ...
      jQuery(document).ready(($) => {
        ...
        viewerJS.jsfAjax.success
          .pipe(
            rxjs.operators.filter(
              (e) => $(e.source).attr("data-select-entry") != undefined
            )
          )
          .subscribe((e) => {
            $(".archives__object-image").hide();
        ...
        viewerJS.jsfAjax.success
          .pipe(
            rxjs.operators.filter(
              (e) => $(e.source).attr("data-expand-entry") != undefined
            )
          )
          .subscribe((e) => {
            $(".archives__object-image").hide();
    ...
    initImageDisplay() {
      ...
      if ($recordPiInput.length > 0) {
        ...
        if (!containsImage || containsImage.toLowerCase() == "false") {
          ...
          let manifestUrl =
            rootURL + "/api/v2/records/" + recordPi + "/manifest/";
                ...
                riot.mount(".archives__object-thumbnails", "thumbnails", {
                  ...
                  link: (canvas) => {
                    if (canvas.homepage && canvas.homepage.length > 0) {
                      return canvas.homepage[0].id.replace(
                        "/image/",
                        "/fullscreen/"
                      );
                    } else {
...
var viewerJS = (function (viewer) {
  ...
  var _bookmarks = {
    ...
    init: function (config) {
      ...
      this.translator = new viewerJS.translator(
        this.config.rest,
    ...
    renderCounter: function () {
      ...
      if ($counter.length > 0) {
        this.listsUpdated
          .pipe(rxjs.operators.merge(rxjs.of("")))
          .subscribe((list) => {
            let count = 0;
    ...
    renderUserBookmarksDropdown: function () {
      ...
      var $dropdownUserLogin = $(
        "<bookmarkListLoggedIn></bookmarkListLoggedIn>"
      );
      ...
      $button.on("click", (event) => {
        if (
          (this.config.userLoggedIn && this.getBookmarkListsCount() > 0) ||
          this.getBookmarksCount() > 0
        ) {
          $(event.currentTarget)
      ...
      $("body").on("click", (event) => {
        ...
        if (
          $target.closest('[data-bookmark-list-type="dropdown"]').length > 0
        ) {
      ...
      $("body").on(
        "click",
        ".login-navigation__bookmarks-trigger",
        function () {
          event.preventDefault();
    ...
    prepareBookmarksPopup: function () {
      ...
      $('[data-bookmark-list-type="add"]')
        ...
        .on(
          "click",
          function (event) {
            var currBtn = $(event.target).closest(
              '[data-bookmark-list-type="add"]'
            );
          ...
          .then((data) => {
            if (data.success === false) {
              return Promise.reject(
                "Failed to remove bookmark: " + data.message
              );
            } else {
            }
          })
    ...
    addBookmarkList: function (name) {
      ...
      return fetch(url, {
        ...
        .then((data) => {
          if (data.success === false) {
            return Promise.reject(
              "Failed to create bookmarklist: " + data.message
            );
          } else {
          }
        })
    ...
    removeBookmarkList: function (id) {
      ...
      return fetch(url, { method: "DELETE" })
        ...
        .then((data) => {
          if (data.success === false) {
            return Promise.reject(
              "Failed to delete bookmarklist: " + data.message
            );
          } else {
          }
        })
    ...
    init: function (config) {
      ...
      $(_defaults.popoverTriggerSelector).on("click", function () {
        ...
        fetch(_defaults.indexResourceUrl, {
          ...
          .catch((error) => {
            console.log(
              "Error calling " + _defaults.indexResourceUrl + " :" + error
            );
          });
      });
    ...
    init: function (config) {
      ...
      $("#chronoSlider").slider({
        ...
        slide: function (event, ui) {
          $(".chronology-slider-start input").val(
            _defaults.yearList[ui.values[0]]
          );
          $(".chronology-slider-end input").val(
            _defaults.yearList[ui.values[1]]
          );
      ...
      // set handler position
      _firstHandlePos = parseInt(
        $("#chronoSlider .ui-slider-handle:first").css("left")
      );
      _lastHandlePos = parseInt(
        $("#chronoSlider .ui-slider-handle:last").css("left")
      );
...
var viewerJS = (function (viewer) {
  ...
  viewer.cookieBanner = {
      ...
      if (this.config.active) {
        ...
        if (this.bannerStatus === undefined) {
          ...
          if (this.config.lastEditedHash === this.bannerHash) {
            if (_debug)
              console.log(
                "last edited hash equals stored value. Banner status = ",
                this.bannerStatus
              );
            // check banner status
    ...
    initHideBanner() {
      ...
      $('[data-set="cookie-banner-accept"]')
        .off()
        .on("click", () => this.hideBanner(true));
      $('[data-set="cookie-banner-decline"]')
        .off()
        .on("click", () => this.hideBanner(false));
    },
    hideBanner(acceptCookies) {
      if (_debug) {
        console.log("EXECUTE: _hideBanner", acceptCookies);
      }
      $(".cookie-banner__info").slideUp(
        function () {
...
var viewerJS = (function (viewer) {
  ...
  viewer.dateSortedFeed = {
    ...
    init: function (config) {
      ...
      dataURL +=
        "/api?action=query&q=PI:*&jsonFormat=datecentric&sortField=DATECREATED";
      dataURL += "&sortOrder=";
  ...
  function _dateConverter(str) {
    ...
    var newDate =
      strArr[2] + ". " + _defaults.monthNames[monthIdx] + " " + strArr[0];
...
var viewerJS = (function (viewer) {
  ...
  viewer.disclaimerModal = {
    ...
    init: function (config) {
      ...
      if (this.config.active) {
        ...
        if (this.settings.lastAccepted) {
          ...
          if (_debug)
            console.log("disclaimer last accepted ", lastAcceptedDate);
          if (_debug)
            console.log(
              "disclaimer valid for session ",
          ...
          if (
            ...
            //accepted disclaimer before last disclaimer update --> need to accept again
            this.showDisclaimer();
          } else {
          ...
          //either the disclaimer has never been shown in this browser or within this session
          this.showDisclaimer();
        }
      }
    },
    ...
    getStoredSettings(location) {
      ...
      if (_debug)
        console.log("retrieve disclaimer setings ", string, " from ", location);
      if (string) {
        try {
          return JSON.parse(string);
        } catch (e) {
          console.error(
            "Error loading disclaimer settings from " + location,
            e
          );
          return {};
        }
    ...
    setStoredSettings(settings, location) {
      if (_debug)
        console.log("save disclaimer setings ", settings, " to ", location);
      if (location && location.toLowerCase() === "session") {
        sessionStorage.setItem(
          "goobi.viewer.disclaimer.settings",
...
var viewerJS = (function (viewer) {
  ...
  viewer.download = {
    ...
    checkboxValidation: function (state) {
      if (_debug) {
        console.log(
          "---------- viewer.download.checkboxValidation() ----------"
        );
...
var viewerJS = (function (viewer) {
  ...
  viewer.fullscreen = {
    ...
    init: function (config) {
      ...
      $("#fullscreenViewImage, #fullscreenViewSidebar").on(
        "mousemove",
        function () {
          _hideHeader(true);
        }
      ...
      $('[data-close="fs-sidebar"]').on("click", function () {
        ...
        $("#fullscreenViewSidebar").animate(
          ...
          function () {
            ...
            let sidebarOpenBtn = sidebarOpenDiv[0].querySelector(
              '[data-open="fs-sidebar"]'
            );
            ...
            if (window.matchMedia("(max-width: 480px)").matches) {
              $(
                ".image-controls__action.prev, .image-controls__action.next"
              ).show();
            }
      ...
      $(".fullscreen__view-sidebar-accordeon-panel-title").on(
        "click keydown",
        function (e) {
          if ((e.type == "keydown" && e.key == "Enter") || e.type == "click") {
            var parentPanelId = $(this).parent().attr("id");
            var panelSessionStatus = JSON.parse(
              sessionStorage.getItem("fsPanelStatus")
            );
            ...
            if ($(this).hasClass("in")) {
              ...
              sessionStorage.setItem(
                "fsPanelStatus",
                JSON.stringify(panelSessionStatus)
              );
            } else {
              $(".fullscreen__view-sidebar-accordeon-panel-title").each(
                function () {
                  $(this).removeClass("in");
                }
              );
              $(".fullscreen__view-sidebar-accordeon-panel-body").each(
                function () {
                  $(this).slideUp("fast");
                }
              );
  ...
  function _hideEmptyPanels() {
    document
      .querySelectorAll(".fullscreen__view-sidebar-accordeon-panel")
      .forEach((panel) => {
        let childCount = [
      ...
      $(".fullscreen__view-sidebar-accordeon-panel").each(function () {
        ...
        if (!panelStatus.hasOwnProperty(currId)) {
          ...
          if (panelStatus[currId]) {
            $(this)
              .find(".fullscreen__view-sidebar-accordeon-panel-title")
              .addClass("in");
            $(this)
              .find(".fullscreen__view-sidebar-accordeon-panel-body")
              .show();
          }
        } else {
          return false;
        ...
        if ($(".fullscreen__view-image-thumbs-wrapper").is(":visible")) {
          setTimeout(function () {
            $(".fullscreen__view-image-thumbs-wrapper").outerWidth(
              $("#fullscreenView").outerWidth() -
                $("#fullscreenViewSidebar").outerWidth()
            );
          }, 325);
        }
      },
  ...
  function _handleKeypress(event) {
    ...
    var doubleKeypress =
      _lastKeyCode == keyCode && now - _lastKeyPress <= _maxDoubleClickDelay;
    ...
    switch (keyCode) {
      ...
      case 27:
        ...
        if (
          $('[data-js="exit-fullscreen"]').length &&
...
var viewerJS = (function (viewer) {
  ...
  function _initFulltextPopovers() {
    ...
    $("[data-entity-type]:not([data-entity-authority-data-uri])").attr(
      "disabled",
      true
    );
    ...
    $("[data-entity-authority-data-uri]").each(function (normdataConfig) {
      // MSG KEYS
      var actionsPopoverHeadingMsg =
        _defaults.msg.fulltextPopoverActionsTermMsg;
      ...
      var thisEntityBody = $(thisEntityElement)
        .next()
        .next()
        .find(".popover-body")
        .html();
      var thisEntityTitle = $(thisEntityElement)
        .next()
        .next()
        .find(".popover-heading")
        .html();
      var thisEntityAfterElement = $(thisEntityElement).next(
        '[data-placeholder="forPopover"]'
      );
  ...
  viewer.groupSelect = {
    ...
    init: function (config) {
      ...
      $(this.config.selectAllSelector).each((index, groupSelectCheckbox) => {
        if (_debug)
          console.log("found select all checkbox", groupSelectCheckbox);
    ...
    initGroup: function (groupName) {
      ...
      rxjs
        ...
        .subscribe((e) => {
          let checked = $(e.target).is(":checked");
          let numChecked = $(
            this.getSelectOneSelector(groupName) + ":checked"
          ).length;
          let numTotal = $(this.getSelectOneSelector(groupName)).length;
          if (_debug)
            console.log(
              numChecked + " out of " + numTotal + " checkboxed checked"
            );
    ...
    getGroupSelector: function (groupName) {
      let groupSelector =
        "[" + this.config.groupAttribute + "='" + groupName + "']";
      return groupSelector;
    },
    getSelectAllSelector: function (groupName) {
      let selector =
        this.getGroupSelector(groupName) + this.config.selectAllSelector;
      return selector;
    },
    getSelectOneSelector: function (groupName) {
      let selector =
        this.getGroupSelector(groupName) + this.config.selectOneSelector;
      return selector;
    },
  ...
  viewer.iiif = {
    ...
    getChildCollections: function (collection) {
      if (collection.service && Array.isArray(collection.service)) {
        ...
        collection.service &&
        collection.service["@context"].endsWith(
          "/collection/extent/context.json"
        )
      ) {
    ...
    getTags: function (collection, name) {
      ...
      if (collection.service && Array.isArray(collection.service)) {
        let tagService = collection.service
          .filter((service) =>
            service["@context"].endsWith("/taglists/context.json")
          )
      ...
      .then((locations) => {
        ...
        if (locations.length > 0) {
          if (!this.map) {
            ...
            if (_debug)
              console.log("set view to ", _getBody(locations[0]).view);
          ...
          this.markers = this.markers.concat(
            _addMarkers(
              this.map,
              locations,
              this.config.msg ? this.config.msg.propertiesLink : undefined
            )
          );
        }
...
var viewerJS = (function (viewer) {
  ...
  viewer.mirador = {
    ...
    init: function (config) {
      ...
      if (manifests.length > 0) {
        ...
        miradorConfigPromise = _getMiradorConfigForManifestUrls(
          manifests,
          this.config
        );
        ...
        miradorConfigPromise = _getUserMiradorObjects(
          this.config.restEndpoint,
          _getBookmarkListId()
        )
          .then((response) => response.json())
        ...
        miradorConfigPromise = _getMiradorSessionObjects(
          this.config.restEndpoint
        )
          .then((response) => response.json())
          .then((json) =>
  ...
  function _getManifestsFromUrlQuery(config) {
    ...
    var piManifests = pis.map(
      (pi) => config.manifestEndpoint + "records/" + pi + "/manifest/"
    );
    ...
    var miradorConfig = {
      ...
      windows: manifests.map((man) => {
        ...
        if (config.startPage) {
          ...
          man.sequences &&
          man.sequences.length > 0 &&
          man.sequences[0].startCanvas
        ) {
...
var viewerJS = (function (viewer) {
    ...
    init: function (config) {
      ...
      $(config.subMenuSelector).on("click", function () {
        ...
        if ($(currTrigger).next(".navigation__submenu").hasClass("in")) {
          ...
          $(currTrigger).parents(".navigation__submenu").hasClass("in")
        ) {
          _resetSubMenus();
      ...
      $("body").on("click", function (event) {
        if (
          ...
          $(event.target).closest(config.navigationSelector).length
        ) {
          return;
        } else {
          _resetMenus();
  ...
  function _calcSubMenuPosition(menu) {
    ...
    if (_debug) {
      console.log(
        "_clacSubMenuPosition: currentOffsetLeft - ",
        currentOffsetLeft
      );
...
var viewerJS = (function (viewer) {
      ...
      if (viewer.localStoragePossible) {
        ...
        $(_defaults.setTagRange).on("change", function () {
          ...
          // render section
            ...
            // setup values
            sessionStorage.setItem("currentNerPageRange", currVal);
            _currentNerPageRange = sessionStorage.getItem(
              "currentNerPageRange"
            );
        ...
        $(_defaults.facettingTrigger).on("click", function () {
          ...
          if (_defaults.currentPage === "nerfacetting") {
            if (_currentNerPageRange == null || _currentNerPageRange === "") {
              _currentNerPageRange = sessionStorage.setItem(
                "currentNerPageRange",
                1
              );
            }
          ...
          if ($(this).parent().parent().parent().attr("id") === "overview") {
            // setup data
            _apiCall = _getAllTagsOfARange(
              _currentNerPageRange,
              _currentNerType
            );
  ...
  function _renderOverview(data) {
    ...
    $.each(data.pages, function (p, page) {
      ...
      if (page.tags.length === 0 || page.tags.length === "undefined") {
        _html +=
          '<span class="page-tag empty">' + _defaults.msg.emptyTag + "</span>";
      } else {
        $.each(page.tags, function (t, tag) {
          _html +=
            '<span class="page-tag ' + tag.type + '">' + tag.value + "</span>";
        });
      }
      _html += "</div>";
      _html += "</li>";
    ...
    $(_defaults.overviewContent)
      ...
      .each(function () {
        $(this)
          .children(".page-tag")
          .slice(_defaults.recurrenceNumber)
          .remove();
      });
  ...
  function _renderSection() {
    if (_debug) {
      ...
      console.log(
        "_renderSection: _currentNerPageRange = ",
        _currentNerPageRange
      );
          ...
          stop: function (event, ui) {
            ...
            switch (_currentNerPageRange) {
              case "5":
                ...
                _apiCall = _getAllTagsOfPageSection(
                  _start,
                  _end,
                  _currentNerType
                );
  ...
  function _renderPageBubble(page) {
    ...
    switch (_currentNerPageRange) {
      case "5":
        ...
        pageBubble +=
          '<span class="page-bubble">' + _start + "-" + _end + "</span>";
        break;
      case "10":
        _start = page - 5;
        _end = page + 5;
  ...
  function _getAllTags() {
    ...
    let url =
      _defaults.baseUrl + _defaults.apiUrl + _defaults.workId + "/ner/tags/";
    return url;
  }
...
var viewerJS = (function (viewer) {
  ...
  function _initNerPopover(path) {
    ...
    $(".ner-trigger").on("click", function () {
      ...
      textBox.append(
        _renderNerPopover(
          data,
          _calculateNerPopoverPosition(triggerCoords, textBoxCoords),
          title,
          path
        )
      );
      ...
      if ($(".ner-popover")) {
        ...
        $(".ner-detail-trigger").on("click", function () {
          ...
          $(this)
            .parent()
            .next(".ner-popover-detail")
            .html(_renderNerPopoverDetail(data, title));
        });
      }
    });
  }
  ...
  function _renderNerPopover(data, position, title, path) {
    ...
    popover +=
      '<div class="ner-popover" style="top:' +
      positionTop +
      "px; left:" +
      positionLeft +
      'px">';
    popover +=
      '<div class="ner-popover-close" title="Fenster schlie&szlig;en">&times;</div>';
      ...
      $.each(object, function (property, value) {
        ...
        $.each(value, function (p, v) {
          ...
          if (v.url) {
            ...
            if (property === "URI") {
              objValue +=
                '<a href="' + v.text + '" target="_blank">' + v.text + "</a>";
            } else {
              objValue += v.text;
            }
          }
  ...
  function _renderNerPopoverDetail(data, title) {
    if (_debug) {
      ...
      console.log(data);
      console.log(
        "viewer.nerFulltext _renderNerPopoverDetail: title - " + title
      );
    }
    ...
    popoverDetail +=
      '<div class="ner-popover-detail-header"><h3>' + title + "</h3></div>";
  ...
  function _calculateNerPopoverPosition(triggerCoords, textBoxCoords) {
    if (_debug) {
      console.log(
        "viewer.nerFulltext _calculateNerPopoverPosition: triggerCoords - "
      );
      console.log(triggerCoords);
      console.log(
        "viewer.nerFulltext _calculateNerPopoverPosition: textBoxCoords - "
      );
...
var viewerJS = (function (viewer) {
  ...
  function _renderPopoverAction($Obj, id) {
    ...
    if (
      $(".normdata-popover-content img").hasClass(
        "normdata-popover-content__icon"
      )
    ) {
    ...
    $.each(data, function (i, object) {
      $.each(object, function (property, value) {
        ...
        $.each(value, function (p, v) {
          ...
          if (v.url) {
            html +=
              '<button type="button" class="normdataDetailLink" data-remotecontent="';
            html += _defaults.path;
...
var viewerJS = (function (viewer) {
  ...
  viewer.notifications = {
    success: (titleAlert, message) =>
      viewer.notifications.notify(titleAlert, message, "success"),
    error: (titleAlert, message) =>
      viewer.notifications.notify(titleAlert, message, "error"),
    warn: (titleAlert, message) =>
      viewer.notifications.notify(titleAlert, message, "warning"),

    confirm: (message, confirmText, denyText, titleAlert, opts = {}) => {
      return viewer.translator
        .addTranslations(["cancel", "ok", "delete"])
        .then(() => {
          confirmText = confirmText
            ? confirmText
            : viewerJS.translator.translate("delete");
          denyText = denyText
            ? denyText
            : viewerJS.translator.translate("cancel");
  ...
  viewer.swaltoasts = {
    success: (titleAlert, message) =>
      viewer.swaltoasts.toast(titleAlert, message, "success"),
    error: (titleAlert, message) =>
      viewer.swaltoasts.toast(titleAlert, message, "error"),
    warn: (titleAlert, message) =>
      viewer.swaltoasts.toast(titleAlert, message, "warn"),
  ...
  function showJsfMessagesAsSweetAlert(e) {
    if (
      e == undefined ||
      (e.responseText && e.responseText.includes('<ul id="messages">'))
    ) {
    ...
    $('[data-require-confirmation="true"]').each((index, element) => {
      ...
      if (!element.hasConfirmationEvent) {
        ...
        element.onclick = (e) => {
          ...
          const alertIcon =
            alertIconName === "warn" ? "warning" : alertIconName;
            ...
            .then(() => {
              if (element.dataset.confirmationTarget) {
                const target = document.querySelector(
                  element.dataset.confirmationTarget
                );
                if (target) {
                  target.click();
                }
  ...
  viewer.overlay.open = function (node, closable, fullscreen, onClose) {
    return new Promise((resolve, reject) => {
      ...
      if ($overlay.length > 0) {
        ...
        let $dismissButtons = $overlay.find("[data-overlay-action='dismiss']");
        let $contentDismissButtons = $node.find(
  ...
  viewer.paginator = {
    ...
    init: function (config) {
      ...
      $(document).ready(function () {
        ...
        var $numericPaginatorLabelWidth = $(
          '[data-paginator="label"]'
        ).outerWidth();
        ...
        $($numericPaginatorInputWrapper).css(
          "width",
          $numericPaginatorLabelWidthPX
        );
      });
    },
    keypressHandler: function (event) {
      ...
      var doubleKeypress =
        ...
        now - viewer.paginator.lastKeyPress <=
        ...
        switch (keyCode) {
          case _backwardKey:
            if (doubleKeypress && viewer.paginator.config.first) {
              ...
              const fallbackFirst = document.querySelector(
                '[data-target="paginatorFirstPage"]'
              );
              if (fallbackFirst) {
                fallbackFirst.click();
              }
              ...
              const fallbackPrevious = document.querySelector(
                '[data-target="paginatorPrevPage"]'
              );
              if (fallbackPrevious) {
          ...
          case _forwardKey:
            if (doubleKeypress && viewer.paginator.config.last) {
              ...
              const fallbackLast = document.querySelector(
                '[data-target="paginatorLastPage"]'
              );
              if (fallbackLast) {
                fallbackLast.click();
              }
              ...
              const fallbackNext = document.querySelector(
                '[data-target="paginatorNextPage"]'
              );
              if (fallbackNext) {
                fallbackNext.click();
              }
    ...
    changePageNumber: function (e) {
      ...
      if (
        ...
        let targetUrl =
          this.config.targetUrlPrefix +
          targetPageNumber +
          this.config.targetUrlSuffix;
        if (_debug) {
    ...
    init: function () {
      $(_triggerElementSelector).each((index, element) => {
        ...
        if ($popover.length === 1) {
          ...
          console.error(
            "Found no popover matching selector '" +
              popoverSelector +
              "'. Cannot initialize popover"
          );
        }
  ...
  viewer.responsiveColumnGallery = {
    ...
    init: function (config) {
      ...
      _promise
        .then(function (imageData) {
          ...
          _smallViewport =
            viewer.responsiveColumnGallery.checkForSmallViewport();
          ...
          if (_defaults.maxColumnCount > 4) {
            ...
            for (var i = 0; i < _defaults.maxColumnCount; i++) {
              _defaults.galleryObject.append(
                viewer.responsiveColumnGallery.renderColumns(
                  _defaults.maxColumnCount
                )
              );
            }
          }
          ...
          if (_defaults.lightbox.active) {
            $(".lightbox-toggle").on("click", function (event) {
              ...
              _lightboxImage =
                viewer.responsiveColumnGallery.prepareLightbox(_parentImage);
              _imageLightbox =
                viewer.responsiveColumnGallery.renderLightbox(_lightboxImage);
              ...
              $(".rcg-lightbox-image img").on("load", function () {
                viewer.responsiveColumnGallery.centerLightbox(
                  $(".rcg-lightbox-body")
                );
    ...
    renderColumns: function (count) {
      if (_debug) {
        ...
        console.log(
          "viewer.responsiveColumnGallery.renderColumns: count = ",
          count
        );
      }
      var column = "";
    ...
    renderImages: function (data) {
      if (_debug) {
        ...
        console.log(
          "viewer.responsiveColumnGallery.renderImages: data = ",
          data
        );
      }
      var image = "";
      ...
      $.each(data, function (i, j) {
        $.each(j, function (m, n) {
          ...
          image += '<a href="' + n.url + '">';
          image +=
            '<div class="rcg-image-overlay" style="background-color:' +
            _defaults.overlayColor +
            '"></div>';
          image += "</a>";
          ...
          if (_defaults.lightbox.active) {
            image +=
              '<div class="lightbox-toggle" title="' +
              _defaults.lang.showLightbox +
              '">';
            image +=
              '<span class="icon-wrapper lightbox-toggle__icon" aria-hidden="true">';
            image +=
              '<svg class="icon" focusable="false"><use href="' +
              _defaults.themePath +
    ...
    fixedHeight: function ($obj) {
      if (_debug) {
        console.log(
          "---------- viewer.responsiveColumnGallery.fixedHeight() ----------"
        );
        console.log(
          "viewer.responsiveColumnGallery.fixedHeight: $obj = ",
          $obj
        );
      }
    ...
    prepareLightbox: function ($obj) {
      if (_debug) {
        console.log(
          "---------- viewer.responsiveColumnGallery.prepareLightbox() ----------"
        );
        console.log(
          "viewer.responsiveColumnGallery.prepareLightbox: $obj = ",
          $obj
        );
      }
      var lightboxData = {};
    ...
    renderLightbox: function (data) {
      if (_debug) {
        console.log(
          "---------- viewer.responsiveColumnGallery.renderLightbox() ----------"
        );
        console.log(
          "viewer.responsiveColumnGallery.renderLightbox: data = ",
          data
        );
      }
      var lightbox = "";
    ...
    centerLightbox: function ($obj) {
      if (_debug) {
        ...
        console.log(
          "viewer.responsiveColumnGallery.centerLightbox: $obj = ",
          $obj
        );
      }
...
var viewerJS = (function (viewer) {
  ...
  viewer.searchAdvanced = {
    ...
    init: function (config) {
      ...
      sessionStorage.setItem(
        "advSearchValues",
        JSON.stringify(_advSearchValues)
      );
  ...
  viewer.searchList = {
    ...
    init: function (config) {
      ...
      function bindModalTriggerState(triggerSelector, modalSelector) {
        ...
        var $triggers = $(triggerSelector).attr("aria-pressed", false);
      ...
      bindModalTriggerState(
        ".search-list__help .btn.btn--icon",
        "#searchHelpModal"
      );
      bindModalTriggerState(
        ".search-list__save-search .btn.btn--icon",
        _defaults.saveSearchModalSelector
      );
      ...
      function setListViewGrid() {
        ...
        $(".search-list__hit-thumbnail img").each(function () {
          $(this).on("load", function (event) {
            ...
            if (imgUrl) {
              $(event.currentTarget)
                .parents(".search-list__hit-thumbnail")
      ...
      if (!_defaults.listStyle) {
        ...
        if (_debug) {
          console.log(
            "default value used (can be based on CMS comp), nothing else defined: " +
              _defaults.listStyle
          );
        }
      }
      ...
      if ($thumbToggle.length > 0) {
        ...
        _searchListShowThumbs =
          sessionStorage.getItem("searchListShowThumbs").toLowerCase() ===
          "true";
    ...
    initSubHits: function () {
      document
        .querySelectorAll('[data-toggle="hit-content"]')
        .forEach((button) => {
          if (
            parseInt(button.dataset.childhits) <=
            _defaults.maxChildHitsToRenderOnStart
          ) {
    ...
    openChildHits: function (button) {
      ...
      let toggleArea = document.querySelector(
        "div[data-toggle-id='" + button.dataset.toggleId + "']"
      );
      let hitsPopulated = toggleArea.querySelector("[data-hits-populated]")
        .dataset.hitsPopulated;
      if (_debug) {
        console.log(
          "clicked hit-content",
          button,
      ...
      if (hitsPopulated < _defaults.childHitsToLoadOnExpand) {
        ...
        this.showAjaxLoader(
          toggleArea.querySelector(".search-list__loader").id
        );
    ...
    initChildHitThumbs: function (dataToggleId) {
      ...
      if (this.isShowChildHitThumbs()) {
        $("[data-toggle-id=" + dataToggleId + "]")
          .find(".search-list__subhit-thumbnail")
          .show();
      } else {
        $("[data-toggle-id=" + dataToggleId + "]")
          .find(".search-list__subhit-thumbnail")
    ...
    isShowChildHitThumbs() {
      return sessionStorage.getItem("searchListShowThumbs").toLowerCase() ===
        "true"
        ? true
        : false;
    },
...
var viewerJS = (function (viewer) {
  ...
  function _setupLightBox(path, name) {
    ...
    lightbox +=
      '<span class="lightbox-close-btn" title="Fenster schlie&szlig;en">&times;</span>';
    lightbox += "</div>";
    lightbox +=
      '<img src="' + path + name + '" alt="' + name + '" /></div></div>';
  ...
  viewer.slider = {
    ...
    init: function () {
      ...
      viewer.jsfAjax.success
        .pipe(
          rxjs.operators.filter(
            (e) => e.responseText && e.responseText.includes("<slider ")
          ),
    ...
    get: function (name) {
      ...
      if (!config) {
        console.warn(
          'Style "' +
            name +
            '" is not included in the list of slider styles. Using "base" as fallback'
        );
  ...
  viewer.stackedThumbnails = {
    ...
    init: function (config) {
      ...
      $(".stacked-thumbnail").on("load", function () {
        $(".stacked-thumbnail-before, .stacked-thumbnail-after").each(function (
          i
        ) {
          if ($(this).isInViewport()) {
            $(this).addClass("-shown");
  ...
  viewer.stickyElements = {
    ...
    initRefresh: function () {
      this.refresh.pipe(rxjs.operators.delay(0)).subscribe(() => {
        ...
        $(document).ready(() =>
          $(".-sticky, .-refreshHCsticky").hcSticky("refresh", {})
        );
      });
...
var viewerJS = (function (viewer) {
  ...
  viewer.loadThumbnails.prototype.loadAll = function () {
    $('[data-viewer-thumbnail="thumbnail"]').each((index, element) =>
      this.load(element)
    );
  };
...
var viewerJS = (function (viewer) {
  ...
  var _defaults = {
    ...
    toolbar: [
      "formatselect | undo redo | bold italic underline strikethrough superscript forecolor backcolor | link | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | fullscreen code | addFeedbackModalLink",
    ],
    menubar: false,
    statusbar: false,
...
var viewerJS = (function (viewer) {
  ...
  viewer.translator.prototype.translate = function (key, language) {
    ...
    if (viewer.isString(key)) {
      ...
      let translation = viewerJS.getMetadataValue(
        this.translations[key],
        language
      );
      if (!translation) {
        translation = viewerJS.getMetadataValue(
          this.translations[key],
  ...
  viewer.userLogin = {
    ...
    init: function () {
      ...
      $("body").on("click", '[data-close="retrieve-account"]', function () {
        $(
          "#loginTypeExternal, #loginTypeRetrieveAccount, #loginTypeCreateAccount"
        ).hide();
        $(
          "#userLoginSelectLoginWrapper, #loginType, #userLoginOpenId, #userLoginCreateAccount"
        ).show();
      });
      ...
      $("#createAccountAcceptTerms input").change(function () {
        if (
          $("#createAccountAcceptTerms input:nth-of-type(2)").is(":checked")
        ) {
    ...
    init: function () {
      ...
      if ($(".-validation-mark").length) {
        viewer.jsfAjax.success.subscribe((e) => {
          ...
          if (_debug)
            console.log("check validation messages ", $validationMessages);
          $validationMessages.each((index, message) => {
            ...
            $message
              .nextAll(".-validation-mark, .-validation-input")
              .addClass(severity);
          });
        });
      }
 ...
 *
 * @version 3.4.0
 * @module viewerJS.widgetAnnotations
 * @description Module to steer widget annotations functionality.
 * @requires jQuery
 */
...
(function () {
  ...
  class ZoomableImageOverlayGroup {
    ...
    constructor(image, overlaySources, config) {
      ...
      this.overlays = createOverlays(
        overlaySources,
        image.getCurrentTileSourceId()
      );
    }
  ...
  class PageAreas {
    constructor(config, image) {
      ...
      let styles = viewerJS.helper.getCss("page-area", [
        "borderTopColor",
        "borderTopWidth",
        "background-color",
      ]);
      ...
      image.viewer.onOpened.subscribe((viewer) => {
        ...
        let activeAreas = this.areas.filter(
          (a) => a.logId === config.currentLogId
        );
        if (activeAreas.length > 0) {
          this.drawActiveAreas(activeAreas, image);
          this.initAreaClick(image, activeAreas);
        } else {
          let inactiveAreas = this.areas.filter(
            (a) => a.logId !== config.currentLogId
          );
          let inactiveLogIds = inactiveAreas
            .map((a) => a.logId)
            .filter((v, i, a) => a.indexOf(v) === i); //last filter to make logIds unique
          inactiveLogIds.forEach((logId) => {
            let logIdAreas = this.areas.filter((a) => a.logId === logId);
            logIdAreas.forEach((area, index) =>
              this.drawArea(area, index, image)
            );
          });
    ...
    drawActiveAreas(activeAreas, imageView) {
      ...
      activeAreas.forEach((activeArea, index) => {
        let rect = ImageView.CoordinateConversion.convertToOpenSeadragonObject(
          activeArea.coords
        );
      ...
      let shadow = new ImageView.ImageFilters.HighlightArea(
        imageView.viewer,
        0.5,
        areasOnCanvas,
        true
    ...
    initAreaClick(imageView, areas) {
      if (areas.length > 0) {
        ...
        const viewerClickPipe = imageView.viewer.onClick.pipe(
          rxjs.operators.map(() => url)
        );
        const areaClicks = [];
        areas.forEach((area) => {
          const areaClickPipe = area.overlay
            ?.onClick()
            .pipe(rxjs.operators.map(() => area.url));
          areaClicks.push(areaClickPipe);
        });

        rxjs
          .merge(viewerClickPipe, ...areaClicks)
          .pipe(
    ...
    drawArea(area, shapeIndex, image, clickToLeave) {
      ...
      area.tooltip = new ImageView.Tooltip(
        area.overlay,
        area.label,
        image.viewer,
        {
  ...
  const _config = {
    elementSelectors: {
      ...
      data: {
        ...
        structure:
          '[data-image="zoomable"] [data-image-data="structureIdentifier"]',
  ...
  class ZoomableImage {
    constructor() {
      const imageElement = document.querySelector(
        _config.elementSelectors.image
      );
      if (imageElement) {
        this.pi = document.querySelector(
          _config.elementSelectors.data.record
        ).textContent;
        this.logId = document.querySelector(
          _config.elementSelectors.data.structure
        ).textContent;
        this.currentPageNo = document.querySelector(
          _config.elementSelectors.data.page
        ).textContent;
        this.pageType = document.querySelector(
          _config.elementSelectors.data.pageType
        ).textContent;
        ...
        this.persistence = new ImageView.ViewPersistence(
          this.zoom,
          this.pageType + "." + this.pi
        );
        ...
        this.tileSourceIdToOrder = Object.fromEntries(
          Object.entries(this.tileSources).map(([order, obj]) => [
            viewerJS.iiif.getId(obj),
            order,
          ])
        );
        ...
        this.overlayGroups = [];
        document
          .querySelectorAll(_config.elementSelectors.data.overlays)
          .forEach((element) => {
            ...
            if (coordsString.trim().length > 0) {
              try {
                ...
                const overlays = new ZoomableImageOverlayGroup(this, coords, {
                  ...
                  showTooltip:
                    element.dataset[_config.datasets.data.showTooltip],
    ...
    load() {
      if (this.viewer) {
        return this.viewer
          .load(
            Object.values(this.tileSources),
            this.getCurrentTileSourceIndex()
          )
          .then((image) => {
    ...
    getTileSourceFromId(id) {
      return this.tileSources
        .values()
        .find((source) => viewerJS.iiif.getId(value) == id);
    }
  ...
  function _drawPageAreas(image) {
    const pageAreaElement = document.querySelector(
      _config.elementSelectors.data.pageAreas
    );
  ...
  function createTileSource() {
    const tileSourcesString = document.querySelector(
      _config.elementSelectors.data.tileSource
    ).textContent;
  ...
  function createFooter(viewer) {
    const footerUrl = document.querySelector(
      _config.elementSelectors.data.footer
    ).textContent;
    if (viewer.viewportMargins.bottom > 0 && footerUrl) {
      const footer = new ImageView.Footer(
        viewer,
        viewer.viewportMargins.bottom
      );
  ...
  function initControls(zoom, rotation) {
    ...
    document
      ...
      .forEach((button) =>
        button.addEventListener("click", (e) => rotation.rotateLeft())
      );
    document
      .querySelectorAll(_config.elementSelectors.controls.rotateRight)
      .forEach((button) =>
        button.addEventListener("click", (e) => rotation.rotateRight())
      );
    document
      .querySelectorAll(_config.elementSelectors.controls.reset)
      .forEach((button) =>
  ...
  function createZoomableImageConfig(imageElement) {
    return {
      ...
      fittingMode: getFittingMode(
        document.querySelector(_config.elementSelectors.data.pageType)
          ?.textContent
      ),
      margins: getMargins(
      ...
      zoom: {
        enabled:
          imageElement.dataset[_config.datasets.image.allowZoom] !== "false",
      },
      sequence: getSequenceSettings(
        imageElement.dataset[_config.datasets.image.viewMode]
      ),
      navigator: {
        enabled:
          imageElement.dataset[_config.datasets.image.showNavigator] === "true",
      },
    };
  }
  ...
  class ShareImageFragment {
    ...
    init(image) {
      ...
      $panels.on("click", (e) => {
        let $panel = $(e.target).closest(
          ".fullscreen__view-sidebar-accordeon-panel h3"
        );
      ...
      $(".share-image-area [data-popover='close']").on("click", (e) => {
        ...
        $(e.target)
          .closest(".fullscreen__view-sidebar-accordeon-panel")
          .find("h3")
          .click();
        $(e.target)
          .closest(".fullscreen__view-sidebar-accordeon-panel")
      ...
      try {
        ...
        var fragmentSelectConfig = {
          removeOldAreas: true,
          drawCondition: (event) =>
            this.active && this.fragmentSelect?.currentOverlay == undefined,
        ...
        this.fragmentSelect = new ImageView.AreaSelect(
          image.viewer,
      ...
      this.$fragmentSelectButton.on("shown.bs.popover", () =>
        this.startFragmentSelect()
      );
      this.$fragmentSelectButton.on("hidden.bs.popover", () =>
        this.endFragmentSelect()
      );
    }
    ...
    startFragmentSelect() {
      ...
      if (this.fragmentSelect) {
        this.fragmentSelect.finishedHook.subscribe((area) => {
          ...
          $('[data-fragment-link="page"]').attr(
            "data-copy-share-image",
            pageUrl
          );
          $('[data-fragment-link="iiif"]').attr(
            "data-copy-share-image",
            imageUrl
          );
    ...
    initImageFragmentLinks(fragment) {
      ...
      if (fragment) {
        ...
        $wrapper
          .find('[data-fragment-link="page"]')
          .attr("data-copy-share-image", pageUrl);
        $wrapper
          .find('[data-fragment-link="iiif"]')
          .attr("data-copy-share-image", imageUrl);
...
var viewerJS = (function (viewer) {
  ...
  viewer.GeoMap = function (config) {
    ...
    this.config.layers.forEach((layer) =>
      this.addFeatureGroup(layer, layer.features)
    );
  ...
  viewer.GeoMap.prototype.init = function (view) {
    ...
    this.map = new L.Map(
      this.config.element ? this.config.element : this.config.mapId,
      {
        zoomControl: false,
    ...
    if (
      this.config.tilesource.toLowerCase() == "mapbox" &&
      this.config.mapBox
    ) {
      let url =
        "https://api.mapbox.com/styles/v1/{1}/{2}/tiles/{z}/{x}/{y}?access_token={3}"
    ...
    rxjs
      ...
      .pipe(rxjs.operators.map((e) => this.getView()))
      .subscribe(this.onMapMove);
    rxjs
      .fromEvent(this.map, "contextmenu")
      .pipe(
        rxjs.operators.map((e) =>
          this.layers[0].createGeoJson(
            e.latlng,
            this.map.getZoom(),
            this.map.getCenter()
          )
    ...
    let allFeatures = this.config.layers
  ...
  viewer.GeoMap.prototype.addFeatureGroup = function (
    featureGroupConfig,
    features
  ) {
    ...
    if (this.initialized) {
      ...
      this.onInitialized.then(() => featureGroup.init(features, false));
    }
    return featureGroup;
  };
  ...
  viewer.GeoMap.prototype.setView = function (view, panning) {
    ...
    view.zoom =
      view.zoom == undefined || Number.isNaN(view.zoom)
        ? 1
        : Math.max(view.zoom, 1);
    if (view.center) {
  ...
  viewer.GeoMap.prototype.getViewAroundFeatures = function (
    ...
    if (!features || features.length == 0) {
      ...
      features
        .map((f) => L.geoJson(f).getBounds())
        .forEach((b) => bounds.extend(b));
      let center = bounds.getCenter();
      let diameter = this.getDiameter(bounds);
      return {
        zoom:
          diameter > 0
            ? Math.max(1, this.map.getBoundsZoom(bounds.pad(zoomPadding)))
            : defaultZoom,
        center: [center.lng, center.lat],
      };
    }
  ...
  viewer.GeoMap.prototype.setViewToFeatures = function (
    features,
    setViewToHighlighted,
    zoom
  ) {
    if (features && features.length > 0) {
      if (!zoom) {
      ...
      let highlightedFeatures = features.filter(
      ...
      if (setViewToHighlighted && highlightedFeatures.length > 0) {
        let viewAroundFeatures = this.getViewAroundFeatures(
          highlightedFeatures,
          zoom,
          0.5
        );
...
var viewerJS = (function (viewer) {
  ...
  var _defaults_featureGroup = {
    ...
    search: {
      ...
      searchUrlTemplate:
        '/viewer/search/-/WKT_COORDS:"Intersects(POINT({lng} {lat})) distErrPct=0"/1/-/-/',
      linkTarget: "_blank",
    },
    heatmap: {
      enabled: false,
  ...
  viewer.GeoMap.featureGroup.prototype.initFeatures = function (features) {
    ...
    this.locations = L.geoJSON([], {
      ...
      onEachFeature: function (feature, layer) {
        ...
        rxjs
          .fromEvent(layer, "click")
          .pipe(rxjs.operators.map((e) => layer.feature))
          .subscribe(this.onFeatureClick);

        let title = viewerJS.getMetadataValue(
          feature.properties.title,
          this.config.language
        );
        let desc = viewerJS.getMetadataValue(
        ...
        if (
          this.config.popover &&
          feature.properties &&
          (title || desc || this.config.emptyMarkerMessage)
        ) {
          if (this.config.popoverOnHover) {
            rxjs
              .fromEvent(layer, "mouseover")
              .subscribe(() => layer.openPopup());
            rxjs
              .fromEvent(layer, "mouseout")
              .subscribe(() => layer.closePopup());
          }
  ...
  viewer.GeoMap.featureGroup.prototype.highlightMarker = function (feature) {
    if (feature) {
      ...
      if (
        this.config.markerIcon.useDefault &&
        this.config.markerIcon.highlightIcon
      ) {
  ...
  viewer.GeoMap.featureGroup.prototype.getMarkerIcon = function (properties) {
    ...
    if (
      this.config.markerIcon &&
      !jQuery.isEmptyObject(this.config.markerIcon)
    ) {
      if (this.config.markerIcon.useDefault) {
        ...
        if (this.config.markerIcon.type == "DivIcon") {
          ...
          options.iconSize = new L.Point(40, 40);
          let icon =
            count > 1 ? this.getClusterIcon(count) : new L.DivIcon(options);
          return icon;
        } else {
          let icon =
            count > 1
              ? this.getClusterIcon(count)
              : L.ExtraMarkers.icon(this.config.markerIcon);
  ...
  viewer.GeoMap.prototype.normalizePoint = function (p) {
    ...
    let wrapped = new L.latLng(
      Math.max(Math.min(p.lat, 90), -90),
      Math.max(Math.min(p.lng, 180), -180)
    );
  ...
  viewer.GeoMap.prototype.getDiameter = function (bounds) {
    if (!bounds || !bounds.isValid()) {
      ...
      let diameter = this.map.distance(
        bounds.getSouthWest(),
        bounds.getNorthEast()
      );
      return diameter;
  ...
  viewer.GeoMap.featureGroup.prototype.createGeoJson = function (
    location,
    zoom,
    center
  ) {
  ...
  viewer.GeoMap.featureGroup.prototype.drawPolygon = function (
    points,
    centerView
  ) {
  ...
  viewer.GeoMap.featureGroup.prototype.drawRectangle = function (
    points,
    centerView
  ) {
    let config = $.extend({ interactive: false }, this.config.style, true);
  ...
  viewer.GeoMap.featureGroup.prototype.drawCircle = function (
    center,
    radius,
    centerView
  ) {
  ...
  function _getAllEntities(featureGroup) {
    let entities = featureGroup.markers.flatMap(
      (m) => m.feature.properties.entities
    );
...
if (typeof L !== "undefined") {
  ...
  L.SolrHeatmapQueryAdapters = {
    default: L.SolrHeatmapBaseQueryAdapter.extend({
      ...
      _solrQuery: function () {
        return (
          this.layer._heatmapUrl +
          "/" +
          this.options.solrRequestHandler +
          "?" +
          this.options.field
        );
      },
  ...
  L.SolrHeatmap = L.GeoJSON.extend({
    ...
    initialize: function (heatmapUrl, featureUrl, featureGroup, options) {
      ...
      _this.queryAdapter = new L.SolrHeatmapQueryAdapters[
        this.options.queryAdapter
      ](this.options, _this);
    ...
    _computeHeatmapObject: function (data) {
      ...
      (_this.facetHeatmap = {}),
        (facetHeatmapArray =
          _this.queryAdapter.responseFormatter(data)[this.options.field]);
    ...
    _expandMarker(marker) {
      ...
      if (visibleOne === marker) {
        ...
        if (bounds && this.queryAdapter.ajaxOptionsForSearchHits) {
          $.ajax(this.queryAdapter.ajaxOptionsForSearchHits(bounds)).then(
            (res) => {
              res.forEach((geoJson) => {
    ...
    _computeIntArrays: function () {
      ...
      _this.lengthX =
        (_this.facetHeatmap.maxX - _this.facetHeatmap.minX) /
        _this.facetHeatmap.columns;
      _this.lengthY =
        (_this.facetHeatmap.maxY - _this.facetHeatmap.minY) /
        _this.facetHeatmap.rows;
  ...
  viewer.GeoMapCms = function (config) {
    ...
    if (highlightDocumentId) {
      ...
      this.config.map.layers
        .map((layer) => layer.features)
        .flat()
        .filter((f) => f.properties.documentId == highlightDocumentId)
        .forEach((f) => (f.properties.highlighted = true));
    }
    if (this.config.pagesToShow) {
      this.config.map.layers
        ...
        .filter(
          (f) =>
            !this.config.pagesToShow.includes(f.properties.page) &&
            f.properties.page !== undefined
        )
        .forEach((f) => (f.properties.visible = false));
    }
...
function _hasVisibleItems(feature) {
  return feature.properties?.entities
    ...
    .filter(
      (e) =>
        e.title != undefined &&
        (typeof e.title == "object" || e.title.length > 0)
    ).length;
}
...
var viewerJS = (function (viewer) {
  ...
  viewer.GeoMapFacet.prototype.init = function (features, view) {
    ...
    $(this.config.buttons.$openEditMode).on("click", (e) =>
      this.openEditMode()
    );
  ...
  viewer.GeoMapFacet.prototype.initHitsLayer = function (features) {
    ...
    let hitsLayer = new viewerJS.GeoMap.featureGroup(
      this.geoMap,
      this.config.map.hitsLayer
    );
  ...
  viewer.GeoMapFacet.prototype.initHeatmap = function () {
    ...
    let heatmap = L.solrHeatmap(
      this.config.heatmap.heatmapUrl,
      this.config.heatmap.featureUrl,
      this.hitsLayer,
      {
        field: "WKT_COORDS",
        type: "clusters",
  ...
  viewer.GeoMapFacet.prototype.initDrawLayer = function () {
    ...
    let drawLayer = new viewerJS.GeoMap.featureGroup(
      this.geoMap,
      this.config.map.areaLayer
    );
    if (this.area) {
      switch (this.area.type) {
        case "polygon":
          ...
          break;
        case "circle":
          this.shape = drawLayer.drawCircle(
            this.area.center,
            this.area.radius,
            true
          );
          break;
        case "rectangle":
          this.shape = drawLayer.drawRectangle(
            [this.area.vertices[0], this.area.vertices[2]],
            true
          );
          break;
      }
  ...
  viewer.GeoMapFacet.prototype.openEditMode = function () {
    viewerJS.overlay
      .open(this.config.editMode.$editModeWrapper, false, true, ($node) => {
        $("body").append($node);
        $node.hide();
      })
      .then((overlay) => {
        ...
        this.mapTag = riot.mount(
          this.config.editMode.$editModeMap,
          "geomapsearch",
          {
            inactive: false,
            area: this.area,
...
var viewerJS = (function (viewer) {
  ...
  viewer.GeoMapSearch.prototype.init = function (view) {
    this.config.opts.onFeatureSelect = (feature) =>
      this.setSearchString(JSON.stringify(feature));
  ...
  cms.init = function () {
    ...
    if (ajaxLoader) {
      viewerJS.jsfAjax.begin.subscribe(
        (data) => (ajaxLoader.style.display = "block")
      );
      viewerJS.jsfAjax.complete.subscribe(
        (data) => (ajaxLoader.style.display = "none")
      );
    }
  };
...
var cmsJS = (function (cms) {
  ...
  cms.createPage = {
    ...
    initSortables: function (config) {
      ...
      config.sortablesConfig.visibleItemList = $(
        config.sortablesConfig.visibleItemList
      );
      config.sortablesConfig.availableItemList = $(
        config.sortablesConfig.availableItemList
      );
      if (config.sortablesConfig.availableItemList.length > 0) {
        ...
        config.sortablesConfig.availableItemList.on(
          "sortbeforestop",
          function (event, ui) {
        ...
        config.sortablesConfig.visibleItemList.on(
          "sortbeforestop",
          function () {
            cmsJS.createPage.disablePreview();
          }
        ...
        if (_debug) console.log("No sortable list elements available");
        return false;
      }
    },
...
var cmsJS = (function (cms) {
  ...
  cms.GeoMapEditor = function (config) {
    ...
    this.geoMap.onMapRightclick
      .pipe(
        rxjs.operators.takeWhile(() => this.config.allowEditFeatures),
        rxjs.operators.map((geojson) => this.addFeature(geojson))
      )
    ...
    this.geoMap.onMapMove.subscribe(() => this.saveView());

    this.onDeleteClick
      .pipe(
        rxjs.operators.takeWhile(() => this.config.allowEditFeatures),
        rxjs.operators.map(() => this.deleteCurrentFeature())
      )
      .subscribe(() => this.saveFeatures());
    ...
    this.onMetadataUpdate
      .pipe(
        rxjs.operators.takeWhile(() => this.config.allowEditFeatures),
        rxjs.operators.map((metadata) =>
          this.updateCurrentFeatureMetadata(metadata)
        )
      )
      .subscribe(() => this.saveFeatures());
  ...
  cms.GeoMapEditor.prototype.init = function (defaultView) {
    this.geoMap.init(this.getView(defaultView)).then(() => {
  ...
  cms.GeoMapEditor.prototype.addFeatureGroup = function (config, features) {
    ...
    featureGroup.onFeatureMove
      .pipe(
        rxjs.operators.takeWhile(() => this.config.allowEditFeatures),
        rxjs.operators.map((geojson) => this.setCurrentFeature(geojson))
      )
      .subscribe(() => this.saveFeatures());
    featureGroup.onFeatureMove.subscribe((f) =>
      console.log("Moving feature ", f)
    );
    return featureGroup;
  };
  ...
  cms.GeoMapEditor.prototype.saveFeatures = function () {
    if (this.activeFeatureGroup) {
      ...
      this.activeFeatureGroup
        .getFeatures()
        ?.forEach((f) => (f.layer = undefined));
      $(this.config.featuresInput).val(
        JSON.stringify(this.activeFeatureGroup.getFeatures())
      );
      this.activeFeatureGroup
        .getFeatures()
        ?.forEach((f) => (f.layer = this.activeFeatureGroup));
    }
  };
   ...
   */
  cms.GeoMapEditor.prototype.setCurrentFeature = function (
    geojson,
    deselectIfCurrent
  ) {
  ...
  cms.GeoMapEditor.prototype.updateCurrentFeatureMetadata = function (
    metadata
  ) {
  ...
  cms.GeoMapEditor.prototype.getView = function (view) {
    if (!view) {
      ...
      if (this.activeFeatureGroup?.getFeatures()?.length > 0) {
        return this.geoMap.getViewAroundFeatures(
          this.activeFeatureGroup.getFeatures(),
          view.zoom
        );
      } else {
        return view;
  ...
  cms.GeoMapEditor.prototype.updateView = function (view) {
    if (!view) {
      ...
      if (this.activeFeatureGroup?.getFeatures()?.length > 0) {
        this.geoMap.setView(
          this.geoMap.getViewAroundFeatures(
            this.activeFeatureGroup.getFeatures(),
            view.zoom
          )
        );
  ...
  cms.GeoMapEditor.prototype.setAllowEditFeatures = function (allow) {
    ...
    if (this.activeFeatureGroup) {
      ...
      this.activeFeatureGroup.markers
        .filter((m) => m.dragging)
        .forEach((marker) => {
          if (allow) {
...
var cmsJS = (function (cms) {
  ...
  function _getCenterCoords(infos) {
    ...
    if (
      infos.centerLocation.longitude != "" ||
      infos.centerLocation.latitude != ""
    ) {
...
var cmsJS = (function (cms) {
  ...
  function _renderMasonryGrid(data, config) {
    ...
    data.mediaItems.forEach(function (item) {
      ...
      let description = viewerJS.getMetadataValue(
        item.description,
        config.language
      );
...
var cmsJS = (function (cms) {
  ...
  cms.media = {
    ...
    initEventListeners: function () {
      ...
      // reset bulk actions
      $('#selectAllMediaItems, input[name*="selectMediaItem"]').on(
        "change",
        function () {
      ...
      if (_defaults.forceGrid) {
        ...
        sessionStorage.getItem("adminCmsMediaGrid") == undefined ||
        sessionStorage.getItem("adminCmsMediaGrid") == ""
      ) {
      ...
      $('[data-switch="list"]').on("click", function () {
        ...
        $("#selectAllMediaItems, #bulkActionSelect")
          .prop("disabled", false)
          .removeClass("disabled");
      ...
      if (_defaults.allowEdit) {
        $(".admin-cms-media__file-image, .admin-cms-media__file-close").on(
          "click",
          function () {
      ...
      $(".admin-cms-media__file")
        .first()
        .find(".admin-cms-media__file-prev")
        .addClass("disabled");
      $(".admin-cms-media__file")
        .last()
        .find(".admin-cms-media__file-next")
        .addClass("disabled");
  ...
  function _bulkActionEdit() {
    ...
    $(".admin-cms-media__file").each(function () {
      var $file = $(this),
        $cbStatus = $file
          .find('input[name*="selectMediaItem"]')
          .prop("checked"),
  ...
  function _setFileCancelClick(modal) {
    ...
    if (modal) {
      ...
      $('[data-action="cancel"]')
        ...
        .on("click", function () {
          $(this)
            .parents(".admin-cms-media__file")
            .find(".admin-cms-media__file-metadata-view")
            .addClass("in");
          $(this)
            .parents(".admin-cms-media__file")
            .find(".admin-cms-media__file-metadata-edit")
            .removeClass("in");
          $(this).parent().removeClass("in");
  ...
  cms.modules = {
    ...
    init: function () {
      ...
      viewerJS.jsfAjax.success.subscribe((data) => {
        if (
          $(".cms-module__option-message, .input_form__option-message").length >
          0
        ) {
    ...
    setValidationStatus: function (source) {
      ...
      var stripedSource, status;

      if (source.indexOf(":") > -1) {
        stripedSource = source.match(/.*:(.*)/)[1];
        status = $('[id*="' + stripedSource + '"]')
          .parents(".cms-module__option-control, .input_form__option_control")
          .find(
            ".cms-module__option-message-status, .input_form__option-message-status"
          );
      } else {
        stripedSource = source;
        status = $("#" + stripedSource)
          .parents(".cms-module__option-control, .input_form__option_control")
          .find(
            ".cms-module__option-message-status, .input_form__option-message-status"
          );
      ...
      if (status.hasClass("success")) {
        $('[id*="' + stripedSource + '"]')
          .parents(".cms-module__option-control, .input_form__option_control")
          .find(
            ".cms-module__option-message-mark, .input_form__option-message-mark"
          )
          .addClass("success");
    ...
    initEventListeners: function () {
      ...
      $("body")
        .off("click.helptext")
        .on("click.helptext", '[data-toggle="helptext"]', function () {
          $(this).toggleClass("in");
          ...
          if (forId) {
            ...
            $input = $(this)
              ...
              .find(
                ".cms-module__option-control, .cms-module__option-dropdown"
              );
            $input.toggleClass("in");
            $input
              .find(".cms-module__option-control-helptext")
              .toggleClass("in");
          }
        });
        ...
        .on(
          ...
          function () {
            if ($(this).hasClass("in")) {
              ...
              $(
                '[data-toggle="available-items"], .cms-menu__available-items-toggle'
              ).each(function () {
      ...
      $("body")
        .off("click.option-dropdown")
        .on(
          "click.option-dropdown",
          '[data-toggle="option-dropdown"]',
          function () {
            $(this).next().slideToggle("fast");
          }
        );
...
var cmsJS = (function (cms) {
  ...
  cms.sortableList = {
    ...
    init: function (indent, allowMultiple, config) {
      if (_debug) {
        ...
        console.log(
          "cmsJS.sortableList.init: inputTextField - ",
          config.sortablesConfig.componentListInput
        );
      }
      ...
      if (_inputField === null) {
        console.warn(
          "Input field for item order not found: No order information can be saved!"
        );
      }
      if (config.sortablesConfig.visibleItemList.length === 0) {
        console.error(
          "No container for active items found. Cannot initialize sortable"
        );
      }
      //            if ( config.sortablesConfig.availableItemList.length === 0 ) {
      ...
      $(config.sortablesConfig.availableItemList).on(
        "sortbeforestop",
        _handleBeforeDropFromAvailable
      );
      $(config.sortablesConfig.visibleItemList).on(
        "sortbeforestop",
        _handleBeforeDropFromVisible
      );
      ...
      $('[data-toggle="edit-visible-item"]').on("click", function () {
        ...
        $(this)
          .parent()
          .next(".cms-menu__visible-item-edit-wrapper")
          .slideToggle("fast");
      });
      ...
      $('[data-toggle="edit-available-item"]').on("click", function () {
        ...
        $(this)
          .parent()
          .next(".cms-menu__available-item-edit-wrapper")
          .slideToggle("fast");
      });
    ...
    decreaseLevel: function (element, applyToNext) {
      if (_debug) {
        ...
        console.log(
          "cmsJS.sortableList.decreaseLevel: applyToNext - ",
          applyToNext
        );
      }
      ...
      if (_getLevel(item) > 0) {
        ...
        if (applyToNext) {
          ...
          while (
            nextItem !== null &&
            _getLevel(nextItem) > _getLevel(item) + 1
          ) {
    ...
    increaseLevel: function (element, applyToNext) {
      if (_debug) {
        ...
        console.log(
          "cmsJS.sortableList.increaseLevel: applyToNext - ",
          applyToNext
        );
      }
      var item = _getJQueryItem(element);
  ...
  function _handleBeforeDropFromAvailable(event, ui) {
    if (_debug) {
      console.log(
        "cmsJS.sortableList _handleBeforeDropFromAvailable: event - ",
        event
      );
      console.log(
        "cmsJS.sortableList _handleBeforeDropFromAvailable: ui - ",
        ui
      );
    }
    ...
    if (
      _allowMultipleOccurances &&
      $item.parent().attr("id") === "visibleItemList"
    ) {
  ...
  function _handleBeforeDropFromVisible(event, ui) {
    if (_debug) {
      console.log(
        "cmsJS.sortableList _handleBeforeDropFromVisible: event - ",
        event
      );
    ...
    if (
      _allowMultipleOccurances &&
      item.parent().attr("id") ===
        $(_config.sortablesConfig.availableItemList).attr("id")
    ) {
      ui.item.remove();
    }
  }
  ...
  function _updateAllSortIcons() {
    ...
    var childrenAL = $(_config.sortablesConfig.availableItemList).children(
      "li"
    );
  ...
  function _updateSortIcons(item) {
    ...
    if (
      parent.attr("id") ===
      $(_config.sortablesConfig.visibleItemList).attr("id")
    ) {
  ...
  function _serializeVisibleItems() {
    ...
    var postData = $(_config.sortablesConfig.visibleItemList).sortable(
      "serialize",
      {
        key: "item",
  ...
  function _fixMenuSave() {
    ...
    var menuEditorOffsetRight =
      $(".cms-menu__editor").offset().left + menuEditorWidth;
    ...
    $(window).on("scroll", function () {
      if (window.pageYOffset + 15 > menuSaveOffsetTop) {
        $(".cms-menu__save")
          .addClass("fixed")
          .css("right", menuSavePositionRight + "px");
      } else {
...
var cmsJS = (function (cms) {
  ...
  cms.tagList = {
    ...
    saveTags: function () {
      ...
      _writeTags(
        cms.tagList.$tagListElement.children("[id$=_item]"),
        cms.tagList.$inputField
      );
    },
    /**
     * Method to ...
    ...
    getTag: function (value) {
      ...
      cmsJS.tagList.getTags().each(function (index, tag) {
        if (
          cmsJS.tagList.getValue(tag).trim().toUpperCase() ===
...
$(document).ready(function () {
  ...
  $("body").one("click", ".blockAfterRadioToggler", function (event) {
    $(".blockAfterRadioToggler input").change(function () {
      if (
        $(this)
          .parent(".admin__radio-switch")
          .find("input:nth-of-type(1)")
          .is(":checked")
      ) {
        $(this)
          .closest(".blockAfterRadioToggler")
          .next(".admin__license-selectable-block")
          .animate(
            {
              opacity: "toggle",
            },
  ...
  $("body").on("click", "#pdf_download_quota_info_box", function () {
    if (
      ...
      $("#pdf_download_quota_info_box")
        .children(".admin__default-block")
        .addClass("-gray-box");
    } else {
      $("#pdf_download_quota_info_box")
        .children(".admin__default-block")
        .removeClass("-gray-box");
    }
  });
  ...
  $("body").on(
    "focusout",
    ".admin__language-tabs-vertical-textarea",
    function () {
      $(this)
        .siblings(".admin__language-tabs-vertical")
        .find(".admin__language-tab-vertical.active a")
  ...
  const baseConfig = {
    ...
    extraKeys: {
      ...
      "Ctrl-D": function (cm) {
        cm.setOption(
          "theme",
          cm.getOption("theme") == "default" ? "dracula" : "default"
        );
      },
      "Ctrl-S": (cm) => {
        // if ( _debug ) {
...
var adminJS = (function (admin) {
  ...
  admin.configEditor = {
    ...
    init: function (config) {
      ...
      if (_debug) {
        console.log("configEditor config", this.config);
      }
    ...
    initWebsocket: function () {
      this.socket = new viewerJS.WebSocket(
        window.location.host,
        window.currentPath,
        viewerJS.WebSocket.PATH_CONFIG_EDITOR_SOCKET
      );
    ...
    isReadOnly: function () {
      return (
        this.config.currentFileIsReadable && !this.config.currentFileIsWritable
      );
    },
    initTooltipHelpers: function () {
      var _this = this;
      $(document).ready(() => {
        $(".-isNotReadable").tooltip({
          title: _this.config.fileNotReadableMsgKey,
          placement: "top",
        });
      });
      ...
      $(".admin__config-editor-backup-single-entry").hover(
        function () {
          $(this)
            .find(".admin__config-editor-backup-single-entry-icon .fa-download")
            .tooltip("show");
        },
        function () {
          $(this)
    ...
    initTextArea: function () {
      ...
      // INIT EDITOR MAIN
      this.cmEditor = adminJS.codemirror(
        targetTextArea,
        type,
        this.isReadOnly(),
        {
      ...
      this.cmEditor.on("change", () => {
        ...
        debounce = setTimeout(() => {
          ...
          if (
            this.cmEditor.doc.changeGeneration() == 1 ||
            startEditorValue == newEditorValue
          ) {
            if (_debug) {
        ...
        $("body").one("click", $($watchedElements), function (event) {
          $($watchedElements)
            ...
            .change(function () {
              ...
              let id = $(this)
                .closest($watchedElements)
                .attr("data-watched-element");
              let keepFunctionsVisible = $(
                '[data-target-visibility="keepVisible"]'
              );
              ...
              switch (true) {
                ...
                case id == "keepFunctions":
                  ...
                  if (clickedWatchedElementYes == true) {
                    ...
                    $($watchedElements)
                      .not(clickedWatchedElement, keepFunctionsVisible)
                      .animate(
                        {
                          height: "hide",
                          opacity: 0,
                    ...
                    $(watchedElementsInputsNo)
                      .not(watchedElementInputThis)
                      .prop("checked", true);
                  }
                  // IF CHECKBOX NO
                  if (clickedWatchedElementYes == false) {
    ...
    init: function () {
      ...
      viewerJS.jsfAjax.success.subscribe(function () {
        ...
        $(".admin__translations-textarea").keyup(function () {
          $(".admin__translations-textarea").each(function (i, obj) {
            if ($(this).val().indexOf("zzz") > -1 || !$(this).val()) {
              ...
              $(this)
                .parent()
                .siblings(".admin__form-label")
                .addClass("admin__form-label-highlight");
            } else {
              $(this).parent().removeClass("admin__form-input-highlight");
              $(this)
                .parent()
                .siblings(".admin__form-label")
                .removeClass("admin__form-label-highlight");
            }
          });

          /* check if global msg key identical on msg key up*/
          var globalKey = $(this)
            .parents(".admin__translations-fields")
            .find(".admin__translations-global-key")
            .text();

          if (globalKey != $(this).val() && globalKey != "") {
            $(this)
              .parents(".admin__translations-fields")
              .find(".admin__translations-global-hint")
              .fadeIn("fast");
          } else {
            $(this)
              .parents(".admin__translations-fields")
        ...
        $(".admin__translations-fields").each(function (i, obj) {
          ...
          if (
            !globalKey == "" &&
            globalKey != $(this).find(".admin__translations-textarea").val()
          ) {
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.prototype.getRegion = function () {
    if (
      ...
      this.target.selector.type == "FragmentSelector"
    ) {
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.AuthorityResource.prototype = Object.create(
    crowdsourcing.Annotation.prototype
  );
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.Metadata.prototype = Object.create(
    crowdsourcing.Annotation.prototype
  );
  ...
  crowdsourcing.Annotation.Metadata.prototype.setValue = function (
    field,
    value
  ) {
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.Plaintext.prototype = Object.create(
    crowdsourcing.Annotation.prototype
  );
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.Richtext.prototype = Object.create(
    crowdsourcing.Annotation.prototype
  );
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Annotation.GeoJson.prototype = Object.create(
    crowdsourcing.Annotation.prototype
  );
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.AreaSelector.prototype.createDrawer = function (imageView) {
    ...
    this.drawer = new ImageView.Draw(
      imageView,
      this.getStyle("red"),
      (e) => e.shiftKey && this.allowDrawing()
    );
    this.drawer.finishedDrawing().subscribe((rect) => {
      ...
      const overlay = new ImageView.Overlay(
  ...
  crowdsourcing.AreaSelector.prototype.createTransformer = function (
    imageView,
    overlay
  ) {
    const config = {
    ...
    transformer
      ...
      .pipe(
        rxjs.operators.map((overlay) => _getResultObject(overlay, imageView))
      )
  ...
  crowdsourcing.AreaSelector.prototype.addOverlay = function (
    annotation,
    viewer
  ) {
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Item = function (item, initialCanvasIndex) {
    ...
    this.questions = item.campaign.questions.map(
      (question) => new Crowdsourcing.Question(question, this)
    );
  ...
  crowdsourcing.Item.prototype.initKeyboardEvents = function () {
    document.addEventListener("keyup", (event) => {
      ...
      switch (keyName) {
        case "ArrowLeft":
          targetIndex = this.getPreviousAccessibleIndex(
  ...
  crowdsourcing.Item.prototype.handleMessage = function (message) {
    ...
    if (status && messageKey) {
      viewerJS.translator.addTranslations(messageKey).then(() => {
  ...
  crowdsourcing.Item.prototype.getNextAccessibleIndex = function (
    currentIndex
  ) {
  ...
  crowdsourcing.Item.prototype.getPreviousAccessibleIndex = function (
    currentIndex
  ) {
    if (this.canvases.length == 1) {
      return undefined;
  ...
  crowdsourcing.Item.prototype.onImageRotated = function (
    eventHandler,
    errorHandler,
    completedHandler
  ) {
    this.imageRotationEvents.subscribe(
      eventHandler,
      errorHandler,
      completedHandler
    );
  ...
  crowdsourcing.Item.prototype.onImageOpen = function (
    eventHandler,
    errorHandler,
    completedHandler
  ) {
    this.imageOpenEvents.subscribe(
      eventHandler,
      errorHandler,
      completedHandler
    );
  ...
  crowdsourcing.Item.prototype.onImageViewChanged = function (
    eventHandler,
    errorHandler,
    completedHandler
  ) {
    this.toggleImageViewEvents.subscribe(
      eventHandler,
      errorHandler,
      completedHandler
    );
  ...
  crowdsourcing.Item.prototype.onAnnotationsReload = function (
    eventHandler,
    errorHandler,
    completedHandler
  ) {
    this.annotationRelaodEvents.subscribe(
      eventHandler,
      errorHandler,
      completedHandler
    );
  ...
  crowdsourcing.Item.prototype.onItemInitialized = function (
    eventHandler,
    errorHandler,
    completedHandler
  ) {
    this.itemInitializedSubject.subscribe(
      eventHandler,
      errorHandler,
      completedHandler
    );
  ...
  crowdsourcing.Item.prototype.initViewer = function (imageSource) {
    ...
    this.currentCanvasIndex = Math.max(
      0,
      Math.min(this.currentCanvasIndex, this.canvases.length - 1)
    );
  ...
  crowdsourcing.Item.prototype.loadImage = function (
    index,
    requireConfirmation
  ) {
    if (index == undefined) {
   ...
   */
  crowdsourcing.Item.prototype.deleteAnnotations = function (
    save,
    pageId,
    questionId
  ) {
   ...
   */
  crowdsourcing.Item.prototype.loadAnnotationPages = function (
    questionId,
    pageId
  ) {
    let save = this.getFromLocalStorage();
    ...
    questions.forEach(function (question) {
      ...
      questionPages.forEach(function (page) {
        ...
        pageToLoad.annotations = pageToLoad.annotations.concat(
          page.annotations
        );
      });
    });
   ...
   */
  crowdsourcing.Item.prototype.saveAnnotations = function (
    pageId,
    questionId,
    annotations
  ) {
  ...
  crowdsourcing.Item.prototype.loadNextItem = function (requireConfirmation) {
    ...
    if (this.dirty && requireConfirmation) {
      promise = viewerJS.notifications.confirm(
        Crowdsourcing.translate("crowdsourcing__confirm_skip_page")
      );
    }
    promise
      .then(() => {
...
var Crowdsourcing = (function (crowdsourcing) {
  ...
  crowdsourcing.Question = function (question, item) {
    ...
    this.colors = Crowdsourcing.frameColors
      ? Crowdsourcing.frameColors
      : COLORS;
  };
  ...
  crowdsourcing.Question.prototype.loadAnnotationsFromLocalStorage =
    function () {
  ...
  crowdsourcing.Question.prototype.setDrawingPermission = function () {
    if (this.areaSelector) {
      if (this.item.isReviewMode()) {
        ...
        this.targetFrequency <= this.annotations.length
      ) {
        this.areaSelector.disableDrawer();
  ...
  crowdsourcing.Question.prototype.initializeView = function (
    createNewAnnotation,
    onAddAnnotation,
    onUpdateAnnotation,
    focus
    ...
    if (focus) {
      this.focusCurrentAnnotation = function () {
        if (
          this.currentAnnotationIndex > -1 &&
          this.annotations &&
          this.annotations.length > this.currentAnnotationIndex
        ) {
      ...
      case Crowdsourcing.Question.Selector.RECTANGLE:
        if (this.areaSelector) {
          ...
          this.annotations.forEach((anno) =>
            this.areaSelector.addOverlay(anno, this.item.image)
          );
  ...
  crowdsourcing.Question.prototype.initAreaSelector = function () {
    this.areaSelector = new Crowdsourcing.AreaSelector(
      this.item,
      true,
      this.colors
    );
  ...
  crowdsourcing.Question.prototype.getImage = function (annotation) {
    return this.getImageUrl(
      annotation.getRegion(),
      this.item.getImageId(this.item.getCurrentCanvas())
    );
  };
  ...
  crowdsourcing.Question.prototype.addAnnotation = function (
    id,
    region,
    color
  ) {
  ...
  crowdsourcing.Question.prototype.saveToLocalStorage = function () {
    ...
    if (
      this.targetSelector == Crowdsourcing.Question.Selector.WHOLE_PAGE ||
      this.targetSelector == Crowdsourcing.Question.Selector.WHOLE_SOURCE
    ) {
  ...
  crowdsourcing.Question.prototype.getAnnotationsFromLocalStorage =
    function () {
      let targetId =
        this.targetSelector == Crowdsourcing.Question.Selector.WHOLE_SOURCE
          ? undefined
