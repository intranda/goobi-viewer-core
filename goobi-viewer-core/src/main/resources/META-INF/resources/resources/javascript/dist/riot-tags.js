...
riot.tag2('thumbnails', '<div ref="thumb" class="thumbnails-image-wrapper {this.opts.index == index ? \'selected\' : \'\'} {getPageStatus(index)}" each="{canvas, index in thumbnails}"><a class="thumbnails-image-link" href="{getLink(canvas)}" aria-label="{getAriaLabel(canvas, index)}" tabindex="{getLink(canvas) ? undefined : \'0\'}" role="{getLink(canvas) ? undefined : \'link\'}" onclick="{handleClickOnImage}" onkeydown="{handleKeydownOnImage}"><img class="thumbnails-image" alt="{getImageAlt(canvas)}" riot-src="{getImage(canvas)}" loading="lazy"><div class="thumbnails-image-overlay"><div class="thumbnails-label">{getValue(canvas.label)}</div></div></a></div>', '', '', function(opts) {
...
this.getImageAlt = function(canvas) {
