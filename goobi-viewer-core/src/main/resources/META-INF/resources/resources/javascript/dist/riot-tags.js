riot.tag2(
    'adminmediaupload',
    '<div class="admin-cms-media__upload-wrapper"><div class="admin-cms-media__upload" ref="dropZone"><div class="admin-cms-media__upload-input"><p> {opts.msg.uploadText} <br><small>({opts.msg.allowedFileTypes}: {fileTypes})</small></p><label for="file" class="btn btn--default">{opts.msg.buttonUpload}</label><input id="file" class="admin-cms-media__upload-file" type="file" multiple="multiple" onchange="{buttonFilesSelected}"></div><div class="admin-cms-media__upload-messages"><div class="admin-cms-media__upload-message uploading"><svg class="admin-cms-media__upload-icon admin-cms-media__upload-icon--spinner" viewbox="0 0 24 24" width="30" height="30" aria-hidden="true"><use riot-href="{getIconHref(\'loader-2\')}"></use></svg> {opts.msg.mediaUploading} </div><div class="admin-cms-media__upload-message success"><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="30" height="30" aria-hidden="true"><use riot-href="{getIconHref(\'check\')}"></use></svg> {opts.msg.mediaFinished} </div><div class="admin-cms-media__upload-message error"><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="30" height="30" aria-hidden="true"><use riot-href="{getIconHref(\'alert-circle\')}"></use></svg><span></span></div></div></div><div if="{this.opts.showFiles}" class="admin-cms-media__list-files {this.uploadedFiles.length > 0 ? \'in\' : \'\'}" ref="filesZone"><div each="{file in this.uploadedFiles}" class="admin-cms-media__list-files__file"><img riot-src="{file}" alt="{getFilename(file)}" title="{getFilename(file)}"><div class="delete_overlay" onclick="{deleteFile}"><svg class="admin-cms-media__delete-icon" viewbox="0 0 24 24" width="30" height="24" aria-hidden="true"><use riot-href="{getIconHref(\'trash\')}"></use></svg></div></div></div></div>',
    '',
    '',
    function (opts) {
        ...
        this.initDrop = function () {
            ...
            dropZone.addEventListener('dragover', (e) => {
                ...
                $(
                    '.admin-cms-media__upload-messages, .admin-cms-media__upload-message.uploading, .admin-cms-media__upload-message.success, .admin-cms-media__upload-message.error'
        ...
        this.uploadFiles = function () {
            ...
            $(
                '.admin-cms-media__upload-messages, .admin-cms-media__upload-message.success, .admin-cms-media__upload-message.error'
            ).removeClass('in-progress');
        ...
        this.fileUploaded = function (fileInfo) {
            $('.admin-cms-media__upload-messages, .admin-cms-media__upload-message.uploading').removeClass(
                'in-progress'
            );
            ...
            setTimeout(function () {
                $(
                    '.admin-cms-media__upload-messages, .admin-cms-media__upload-message.uploading, .admin-cms-media__upload-message.success'
                ).removeClass('in-progress');
            }, 5000);
        }.bind(this);
        ...
        this.deleteFile = function (data) {
            this.deleteUploadedFile(data.item.file).then(() => {
                this.getUploadedFiles();
            });
        }.bind(this);
            ...
            return fetch(this.opts.postUrl + this.files[i].name, {
                ...
                .then((exists) => {
                    if (exists) {
                        ...
                        return viewerJS.notifications
                            .confirm('', this.opts.msg.button__overwrite, undefined, message, {
                                icon: 'warning',
...
riot.tag2(
    'bookmarklist',
    '<ul class="{mainClass} list"><li each="{bookmarkList in getBookmarkLists()}"><button if="{pi}" class="btn btn--clean" type="button" onclick="{inList(bookmarkList, this.pi, this.page, this.logid) ? remove : add}"><svg if="{inList(bookmarkList, this.pi, this.page, this.logid)}" class="bookmark-popup__check-icon" viewbox="0 0 24 24" width="16" height="16" aria-hidden="true"><use riot-href="{getIconHref(\'check\')}"></use></svg> {bookmarkList.name} <span>{bookmarkList.numItems}</span></button><div if="{!pi}" class="row no-margin"><div class="col-9 no-padding"><a href="{opts.bookmarks.getBookmarkListUrl(bookmarkList.id)}">{bookmarkList.name}</a></div><div class="col-2 no-padding icon-list"><a if="{maySendList(bookmarkList)}" href="{sendListUrl(bookmarkList)}" title="{msg(\'bookmarkList_session_mail_sendList\')}"><svg class="bookmark-popup__action-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'send\')}"></use></svg></a><a href="{searchListUrl(bookmarkList)}" data-toggle="tooltip" data-placement="top" data-original-title="" title="{msg(\'action__search_in_bookmarks\')}"><svg class="bookmark-popup__action-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'search\')}"></use></svg></a><a href="{miradorUrl(bookmarkList)}" target="_blank" title="{msg(\'viewMiradorComparison\')}"><svg class="bookmark-popup__action-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'grid-dots\')}"></use></svg></a></div><div class="col-1 no-padding"><span class="{mainClass}-counter">{bookmarkList.numItems}</span></div></div></li></ul>',
    '',
    '',
    function (opts) {
        const ensureTrailingSlash = (value) => (value.endsWith('/') ? value : `${value}/`);
        ...
        this.mainClass =
            this.opts.style && this.opts.style.mainClass ? this.opts.style.mainClass : 'bookmark-popup__body-list';

        this.on('mount', function () {
            this.opts.bookmarks.listsUpdated
        ...
        this.add = function (event) {
            ...
            this.opts.bookmarks.addToBookmarkList(list.id, pi, page, logid).then(() => this.updateLists());
        }.bind(this);
        ...
        this.remove = function (event) {
            if (this.opts.bookmarks.config.userLoggedIn) {
                ...
                let logid = undefined;
                let page = this.opts.bookmarks.isTypePage() ? this.page : undefined;
                this.opts.bookmarks.removeFromBookmarkList(list.id, pi, page, logid).then(() => this.updateLists());
            } else {
                let bookmark = event.item.bookmark;
                this.opts.bookmarks
                    .removeFromBookmarkList(0, bookmark.pi, undefined, undefined)
...
riot.tag2(
    ...
    '<ul if="{opts.bookmarks.config.userLoggedIn}" class="{mainClass}-small-list list"><li class="{mainClass}-entry" each="{bookmarkList in getBookmarkLists()}"><div class="login-navigation__bookmarks-name"><a href="{opts.bookmarks.getBookmarkListUrl(bookmarkList.id)}">{bookmarkList.name}</a></div><div class="login-navigation__bookmarks-icon-list icon-list"><a href="{searchListUrl(bookmarkList)}" data-toggle="tooltip" data-placement="top" data-original-title="" title="{msg(\'action__search_in_bookmarks\')}"><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'search\')}"></use></svg></a><a href="{miradorUrl(bookmarkList)}" target="_blank" title="{msg(\'viewMiradorComparison\')}"><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'grid-dots\')}"></use></svg></a><span title="{msg(\'admin__crowdsourcing_campaign_statistics_numRecords\')}" class="{mainClass}-counter">{bookmarkList.numItems}</span></div></li><li class="{mainClass}-entry"><a class="login-navigation__bookmarks-overview-link" href="{allBookmarksUrl()}" data-toggle="tooltip" data-placement="top" data-original-title="" title="{msg(\'bookmarkList_overview_all\')}">{msg(\'bookmarkList_overview_all\')} </a></li></ul>',
    '',
    '',
    function (opts) {
        const ensureTrailingSlash = (value) => (value.endsWith('/') ? value : `${value}/`);
        ...
        this.add = function (event) {
            let list = event.item.bookmarkList;
            this.opts.bookmarks
                .addToBookmarkList(list.id, this.pi, this.page, this.logid, this.opts.bookmarks.isTypePage())
                .then(() => this.updateLists());
        }.bind(this);
        ...
        this.remove = function (event) {
            let list = event.item.bookmarkList;
            this.opts.bookmarks
                .removeFromBookmarkList(list.id, this.pi, this.page, this.logid, this.opts.bookmarks.isTypePage())
                .then(() => this.updateLists());
        }.bind(this);
        ...
        this.deleteList = function (event) {
            let list = event.item.bookmarkList;
            this.opts.bookmarks.removeBookmarkList(list.id).then(() => this.updateLists());
        }.bind(this);
...
riot.tag2(
    'bookmarklistsession',
    '<ul each="{bookmarkList in getBookmarkLists()}" class="{mainClass} list"><li each="{bookmark in bookmarkList.items}"><div class="row no-margin {mainClass}-single-entry"><div class="col-11 no-padding {mainClass}-title"><a href="{opts.bookmarks.config.root}{bookmark.url}"><div class="row no-gutters"><div class="col-4 no-padding"><div class="{mainClass}-image" riot-style="background-image: url({bookmark.representativeImageUrl});"></div></div><div class="col-7 no-padding"><h3>{bookmark.name}</h3></div></div></a></div><div class="col-1 no-padding {mainClass}-remove"><button class="btn btn--clean" type="button" data-bookmark-list-type="delete" onclick="{remove}" aria-label="{msg(\'bookmarkList_removeFromBookmarkList\')}"><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'ban\')}"></use></svg></button></div></div></li></ul><div each="{bookmarkList in getBookmarkLists()}" class="{mainClass}-actions"><div if="{mayEmptyList(bookmarkList)}" class="{mainClass}-reset"><button class="btn btn--clean" type="button" data-bookmark-list-type="reset" onclick="{deleteList}"><span>{msg(\'bookmarkList_reset\')}</span><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'trash\')}"></use></svg></button></div><div if="{maySendList(bookmarkList)}" class="{mainClass}-send"><a href="{sendListUrl(bookmarkList)}"><span>{msg(\'bookmarkList_session_mail_sendList\')}</span><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'send\')}"></use></svg></a></div><div if="{maySearchList(bookmarkList)}" class="{mainClass}-search"><a href="{searchListUrl(bookmarkList)}" data-toggle="tooltip" data-placement="top" data-original-title="" title=""><span>{msg(\'action__search_in_bookmarks\')}</span><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'search\')}"></use></svg></a></div><div if="{mayCompareList(bookmarkList)}" class="{mainClass}-mirador"><a href="{miradorUrl(bookmarkList)}" target="_blank"><span>{msg(\'viewMiradorComparison\')}</span><svg class="admin-cms-media__upload-icon" viewbox="0 0 24 24" width="18" height="18" aria-hidden="true"><use riot-href="{getIconHref(\'grid-dots\')}"></use></svg></a></div></div>',
    '',
    '',
    function (opts) {
        const ensureTrailingSlash = (value) => (value.endsWith('/') ? value : `${value}/`);
        ...
        this.remove = function (event) {
            let bookmark = event.item.bookmark;
            this.opts.bookmarks
                .removeFromBookmarkList(0, bookmark.pi, undefined, undefined, false)
                .then(() => this.updateLists());
        }.bind(this);

        this.deleteList = function (event) {
            this.opts.bookmarks.removeBookmarkList(0).then(() => this.updateLists());
        }.bind(this);
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.add = function () {
            ...
            this.opts.bookmarks.addBookmarkList(name).then(() => {
                this.opts.bookmarks.listsNeedUpdate.next();
                this.update();
            });
        }.bind(this);
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.setStartYear = function (e) {
            ...
            this.startYear = Math.min.apply(
                Math,
                this.yearList.filter((x) => x >= year)
            );
            this.fixDateOrder();
        }.bind(this);
        ...
        this.setRange = function () {
            ...
            Array.from(document.getElementsByClassName('chronology-slider__input-start')).forEach(
                (element) => (element.disabled = true)
            );
            Array.from(document.getElementsByClassName('chronology-slider__input-end')).forEach(
                (element) => (element.disabled = true)
            );
            Array.from(document.getElementsByClassName('chronology-slider__ok-button')).forEach(
                (element) => (element.disabled = true)
            );
            ...
            let options = {
                ...
                slide: (event, ui) => {
                    ...
                    if (this.rtl) {
                        ...
                        this.$getLastHandle().css(
                            'margin-left',
                            -1 *
                                this.$getLastHandle().width() *
                                (ui.values[1] / this.$getSlider().slider('option', 'max'))
                        );

                        this.$getFirstHandle().css(
                            'margin-left',
                            -1 *
                                this.$getFirstHandle().width() *
                                (ui.values[0] / this.$getSlider().slider('option', 'max'))
                        );
                    }
                },
                change: (event, ui) => {
            ...
            if (this.rtl) {
                ...
                $(this.refs.slider)
                    ...
                    .css(
                        'margin-left',
                        -1 *
                            $(this.refs.slider).find('.ui-slider-handle').last().width() *
                            ($(this.refs.slider).slider('values', 1) / $(this.refs.slider).slider('option', 'max'))
                    );
                $(this.refs.slider)
                    .find('.ui-slider-handle')
                    .first()
                    .css(
                        'margin-left',
                        -1 *
                            $(this.refs.slider).find('.ui-slider-handle').first().width() *
                            ($(this.refs.slider).slider('values', 0) / $(this.refs.slider).slider('option', 'max'))
                    );
            }
        }.bind(this);
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.loadSubCollections = function () {
            ...
            observable.pipe(rxjs.operators.debounceTime(100)).subscribe(() => this.update());
        }.bind(this);
        ...
        this.fetchMembers = function (collection) {
            if (this.hasChildren(collection)) {
                return fetch(collection['@id'])
                    ...
                    .then((json) => {
                        collection.members = json.members;
...
riot.tag2(
    'collectionview',
    '<div each="{set, index in collectionSets}"><h2 if="{set[0] != \'\'}">{translator.translate(set[0])}</h2><collectionlist collections="{set[1]}" language="{opts.language}" defaultlanguage="{opts.defaultlanguage}" setindex="{index}" depth="{opts.depth}"></collectionlist></div>',
    '',
    '',
    function (opts) {
        this.collectionSets = [];
    ...
    function (opts) {
        ...
        this.on('mount', () => {
            ...
            this.ws.onOpen.subscribe(() => {
                rxjs.from(this.urls)
                    .pipe(
                        rxjs.operators.flatMap((url) =>
                            this.sendMessage(this.createSocketMessage(this.pi, url, 'status'))
                        )
                    )
                    .subscribe(() => {});
            });
        ...
        this.initWebSocket = function () {
            const socket = new viewerJS.WebSocket(
                window.location.host,
                this.contextPath,
                viewerJS.WebSocket.PATH_DOWNLOAD_TASK
            );
        ...
        this.sendMessage = function (message) {
            ...
            return new Promise((resolve, reject) => {
                rxjs.merge(this.ws.onMessage, this.ws.onError)
        ...
        this.startDownloadTask = function (e) {
            ...
            if (urlToDownload) {
                ...
                this.sendMessage({
                    pi: this.pi,
                    url: urlToDownload,
                    urlTemplate: this.urlTemplates[urlToDownload],
                    action: 'startdownload',
                });
                const listener = viewerJS.helper.repeatPromise(
                    () => this.sendMessage(this.createSocketMessage(this.pi, urlToDownload, 'update')),
                    this.updateDelay
                );
                ...
                switch (data.status) {
                    ...
                    case 'processing':
                        ...
                        if (!this.updateListeners.has(data.url)) {
                            const listener = viewerJS.helper.repeatPromise(
                                () => this.sendMessage(this.createSocketMessage(this.pi, data.url, 'update')),
                                this.updateDelay
                            );
...
riot.tag2(
    'fsthumbnailimage',
    '<div class="fullscreen__view-image-thumb-preloader" if="{preloader}"></div><img ref="image" alt="Thumbnail Image">',
    '',
    '',
    function (opts) {
                ...
                $('[data-show="thumbs"]').on(
                    ...
                    function (e) {
                        ...
                        if (e.currentTarget.classList.contains('in')) {
                            $('[data-show="thumbs"]')
                                .attr('title', opts.msg.hideThumbs)
                                .tooltip('_fixTitle')
                                .tooltip('show');
                        } else {
                            $('[data-show="thumbs"]')
                                .attr('title', opts.msg.showThumbs)
                                .tooltip('_fixTitle')
                                .tooltip('show');
                        }
...
riot.tag2(
    'geomapsearch',
    '<yield><div class="geo-map__wrapper"><div ref="geocoder" class="geocoder"></div><div class="geo-map__buttons-wrapper"></div><div ref="map" class="geo-map"></div></div>',
    '',
    '',
    function (opts) {
        this.on('mount', function () {
            ...
            switch (type) {
                ...
                case 'rectangle':
                    ...
                    let vertices = [...layer.getLatLngs()[0]]
                        .map((p) => L.latLng(p.lat, p.lng - distance))
                        .map((p) => this.geoMap.normalizePoint(p));
                ...
                case 'circle':
                    ...
                    let circumgon = this.createCircumgon(
                        bounds.getCenter(),
                        bounds.getSouthWest(),
                        bounds.getSouthWest(),
                        16
        ...
        this.initHeatmap = function (hitsLayer) {
            ...
            let heatmap = L.solrHeatmap(
                this.opts.hitsLayer.heatmap.heatmapUrl,
                this.opts.hitsLayer.heatmap.featureUrl,
                hitsLayer,
                {
                    field: 'WKT_COORDS',
                    type: 'clusters',
    ...
    function (opts) {
        ...
        this.on('update', () => {
            ...
            if (this.refs.dropdown) {
                this.refs.dropdown.value = this.isDoublePageMode()
                    ? this.currentPageNumbers[0] + '-' + this.currentPageNumbers[0]
                    : this.currentPageNumbers[0];
            }
        });
...
riot.tag2(
    'modal',
    '<div class="modal fade {modalClass}" id="{modalId}" tabindex="-1" ref="modal" role="dialog" aria-labelledby="{modalTitle}" aria-hidden="true"><div class="modal-dialog modal-dialog-centered" role="document"><div class="modal-content"><div class="modal-header"><h1 class="modal-title">{modalTitle}</h1><button class="fancy-close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">x</span></button></div><div class="modal-body"><yield from="body"></yield></div><div class="modal-right"><yield from="right"></yield></div><div class="modal-footer"><yield from="footer"></yield></div></div></div><div class="alt-backdrop"></div></div>',
    '',
    '',
    function (opts) {
...
riot.tag2(
    'pdfdocument',
    '<div class="pdf-container"><pdfpage each="{page, index in pages}" page="{page}" pageno="{index+1}"></pdfPage></div>',
    '',
    '',
    function (opts) {
        this.pages = [];
...
riot.tag2(
    'pdfpage',
    '<div class="page" id="page_{opts.pageno}"><canvas class="pdf-canvas" id="pdf-canvas_{opts.pageno}"></canvas><div class="text-layer" id="pdf-text_{opts.pageno}"></div><div class="annotation-layer" id="pdf-annotations_{opts.pageno}"></div></div>',
    '',
    '',
    function (opts) {
        this.on('mount', function () {
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', () => {
            ...
            if (
                this.opts.mimeTypes[this.currentItem] !== undefined &&
                !this.opts.mimeTypes[this.currentItem].startsWith('image')
            ) {
    ...
    function (opts) {
        ...
        this.getThumbnail = function (manifest, width, height) {
            if (!manifest.thumbnail.service || (!width && !height)) {
                ...
                return (
                    manifest.thumbnail.service['@id'] + '/full/' + sizePrefix + width + ',' + height + '/0/default.jpg'
                );
            }
        }.bind(this);
...
riot.tag2(
    'thumbnails',
    '<div ref="thumb" class="thumbnails-image-wrapper {this.opts.index == index ? \'selected\' : \'\'} {getPageStatus(index)}" each="{canvas, index in thumbnails}"><a class="thumbnails-image-link" href="{getLink(canvas)}" onclick="{handleClickOnImage}"><img class="thumbnails-image" alt="{getObjectTitle() + \': \' + getValue(canvas.label)}" riot-src="{getImage(canvas)}" loading="lazy"><div class="thumbnails-image-overlay"><div class="thumbnails-label">{getValue(canvas.label)}</div></div></a></div>',
    '',
    '',
    function (opts) {
        this.thumbnails = [];
        ...
        this.getImageUrl = function (resource, size) {
            if (size && resource.service && (!Array.isArray(resource.service) || resource.service.length > 0)) {
                let url = viewerJS.iiif.getId(
                    viewerJS.iiif.getId(resource.id) ? resource.service[0] : resource.service
                );
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.getAccessDeniedThumbnailUrl = function (manifest) {
            try {
...
riot.tag2(
    'authorityresource',
    '<div class="annotation__body__authority"><div if="{normdataList.length == 0}">{authorityId}</div><dl class="annotation__body__authority__normdata_list" each="{normdata in normdataList}"><dt class="normdata_list__label">{normdata.property}: </dt><dd class="normdata_list__value">{normdata.value}</dd></dl></div>',
    '',
    '',
    function (opts) {
        this.normdataList = [];
        ...
        this.on('mount', () => {
            ...
            this.url =
                this.opts.resturl +
                'authority/resolver?id=' +
                this.unicodeEscapeUri(this.authorityId) +
                '&template=ANNOTATION&lang=' +
                this.opts.currentlang;
            this.update();
            fetch(this.url)
                .then((response) => {
                    if (!response.ok) {
...
riot.tag2(
    'datasetresource',
    '<div class="annotation__body__dataset"><dl class="annotation__body__dataset__data_list" each="{field in dataFields}"><dt class="data_list__label">{getName(field)}: </dt><dd class="data_list__value">{getValue(field)}</dd></dl></div>',
    '',
    '',
    function (opts) {
        this.dataSet = {};
        ...
        this.on('mount', () => {
            ...
            if (viewerJS.translator) {
                viewerJS.translator.addTranslations(this.dataFields).then(() => this.update());
            } else {
...
riot.tag2(
    'geomapresource',
    '<div id="geomap_{opts.annotationid}" class="annotation__body__geomap geomap"></div>',
    '',
    '',
    function (opts) {
        this.on('mount', () => {
...
riot.tag2(
    'htmltextresource',
    '<div ref="container" class="annotation__body__htmltext"></div>',
    '',
    '',
    function (opts) {
        this.on('mount', () => {
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            this.question.initializeView(
                (anno) => new Crowdsourcing.Annotation.AuthorityResource(anno, this.question.authorityData.context),
                this.update,
                this.update,
                this.focusAnnotation
            );
            this.opts.item.onImageOpen(
                function () {
        ...
        this.showInstructions = function () {
            return (
                !this.opts.item.isReviewMode() &&
                this.question.active &&
                this.question.targetSelector == Crowdsourcing.Question.Selector.RECTANGLE &&
                this.question.annotations.length == 0
            );
        }.bind(this);
        ...
        this.showInactiveInstructions = function () {
            return (
                ...
                !this.question.active &&
                this.question.targetSelector == Crowdsourcing.Question.Selector.RECTANGLE &&
                this.opts.item.questions.filter((q) => q.isRegionTarget()).length > 1
            );
        }.bind(this);
    ...
    function (opts) {
        ...
        this.loadItem = function (itemConfig) {
            ...
            if (this.opts.currentuserid) {
                this.item.setCurrentUser(
                    this.opts.currentuserid,
                    this.opts.currentusername,
                    this.opts.currentuseravatar
                );
            }
        ...
        this.acceptReview = function () {
            this.setStatus('FINISHED').then(() => this.skipItem());
        }.bind(this);

        this.acceptReviewForPage = function () {
            this.setPageStatus('FINISHED').then(() => this.skipPage());
        }.bind(this);

        this.rejectReview = function () {
            this.setStatus('ANNOTATE').then(() => this.skipItem());
        }.bind(this);

        this.rejectReviewForPage = function () {
            this.setPageStatus('ANNOTATE').then(() => this.skipPage());
        }.bind(this);
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.useMiddleButtons = function () {
            return (
                this.getTotalImageCount() > 9 &&
                this.getCurrentIndex() > 4 &&
                this.getCurrentIndex() < this.getTotalImageCount() - 5
            );
        }.bind(this);
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            this.opts.item.onItemInitialized(() => {
                this.question.initializeView(
                    (anno) => new Crowdsourcing.Annotation.GeoJson(anno),
                    this.addAnnotation,
        ...
        this.showInstructions = function () {
            return (
                !this.addMarkerActive &&
                !this.opts.item.isReviewMode() &&
                this.question.active &&
                this.question.isRegionTarget()
            );
        }.bind(this);
        ...
        this.initMap = function () {
            ...
            this.geoMap.onMapClick.subscribe((geoJson) => {
                if (
                    this.addMarkerActive &&
                    (this.question.targetFrequency == 0 ||
                        this.geoMap.layers[0].getMarkerCount() < this.question.targetFrequency)
                ) {
...
riot.tag2(
    ...
    function (opts) {
            ...
            try {
                ...
                if (this.opts.item) {
                    ...
                    this.opts.item.setImageSource = function (source) {
                        this.update();
                        this.image.load(this.getImageInfo(source)).then((e) => this.zoom.goHome());
                    }.bind(this);
                }
            } catch (error) {
                console.error('ERROR ', error);
        ...
        $(document).ready(function () {
            ...
            $(
                '.thumbnails-filter-reset, .thumbnails-filter-finished, .thumbnails-filter-unfinished, .thumbnails-filter-annotated'
            ).click(function () {
                $(
                    '.thumbnails-filter-reset, .thumbnails-filter-finished, .thumbnails-filter-unfinished, .thumbnails-filter-annotated'
        ...
        $(
            '.image_thumbnails-wrapper.reviewmode .thumbnails-image-wrapper:not(".image_thumbnails-wrapper.reviewmode .thumbnails-image-wrapper.finished")'
        ).tooltip('dispose');

        this.getPosition = function () {
            let pos_os = this.dataPoint.getPosition();
            let pos_image = ImageView.CoordinateConversion.scaleToImage(
                pos_os,
                this.image.viewer,
                this.image.getOriginalImageSize()
            );
            let pos_image_rot = ImageView.CoordinateConversion.convertPointFromImageToRotatedImage(
                pos_image,
                this.image.controls.getRotation(),
                this.image.getOriginalImageSize()
            );
        ...
        this.handleImageControlAction = function (event) {
            ...
            switch (event.action) {
                ...
                case 'setImageIndex':
                    ...
                    break;
                case 'previousImage':
                    this.opts.item.loadImage(
                        this.opts.item.getPreviousAccessibleIndex(this.opts.item.currentCanvasIndex),
                        true
                    );
                    break;
                case 'nextImage':
                    this.opts.item.loadImage(
                        this.opts.item.getNextAccessibleIndex(this.opts.item.currentCanvasIndex),
                        true
                    );
                    break;
            }
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            ...
            this.question.initializeView(
                (anno) => new Crowdsourcing.Annotation.Metadata(anno, this.originalData),
                this.update,
                this.update,
                this.focusAnnotation
            );
            ...
            Crowdsourcing.translator.addTranslations(this.question.metadataFields).then(() => this.update());
        });
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            this.question.initializeView(
                (anno) => new Crowdsourcing.Annotation.Plaintext(anno),
                this.update,
                this.update,
                this.focusAnnotation
            );
...
riot.tag2(
    'progressbar',
    '<div class="goobi-progress-bar-wrapper"><div class="goobi-progress-bar"><div each="{value, index in this.values}" class="goobi-progress-bar__bar {styleClasses[index]}" riot-style="width: {getRelativeWidth(value)};"></div></div></div>',
    '',
    '',
    function (opts) {
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            this.question.initializeView(
                (anno) => new Crowdsourcing.Annotation.Implementation(anno),
                this.update,
                this.update,
                this.focusAnnotation
            );
...
riot.tag2(
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            this.question.initializeView(
                (anno) => new Crowdsourcing.Annotation.Richtext(anno),
                this.update,
                this.update,
                this.focusAnnotation
            );
...
riot.tag2(
    'featuresetfilter',
    '<div if="{filters.length > 0}"><div each="{filter in filters}" class="{filter.styleClass}"><label>{filter.label}</label><div><input type="radio" name="options_{filter.field}" id="options_{filter.field}_all" value="" checked onclick="{resetFilter}"><label for="options_{filter.field}_all">{opts.msg.alle}</label></div><ul class="geomap__feature-options-list"><li each="{option, index in filter.options}" class="geomap__feature-options-list-entry"><input type="radio" name="options_{filter.field}" id="options_{filter.field}_{index}" riot-value="{option.name}" onclick="{setFilter}"><label for="options_{filter.field}_{index}">{option.name}</label></li></ul></div></div>',
    '',
    '',
    function (opts) {
        this.filters = [];
        ...
        this.findValues = function (featureGroups, filterField) {
            return Array.from(
                new Set(
                    this.findEntities(featureGroups, filterField)
                        .map((e) => e[filterField])
                        .map((a) => a[0])
                        .map((value) => viewerJS.iiif.getValue(value, this.opts.locale, this.opts.defaultLocale))
                        .filter((e) => e)
                )
            );
        }.bind(this);
        ...
        this.findEntities = function (featureGroups, filterField) {
            let entities = featureGroups
                .flatMap((group) => group.markers)
                .filter((m) => m.feature.properties.entities)
                .flatMap((m) => m.feature.properties.entities)
                .filter((e) => e[filterField]);
            return entities;
        }.bind(this);
                ...
                .map((filter) => {
                    if (filter.selectedValue) {
                        let show =
                            ...
                            entity[filter.field]
                                .map((v) => viewerJS.iiif.getValue(v, this.opts.locale, this.opts.defaultLocale))
                                .includes(filter.selectedValue);
                        return show;
                    } else {
                        return true;
                    }
...
riot.tag2(
    'featuresetselector',
    '<div class="tab" if="{featureGroups.length > 1}"><button each="{featureGroup, index in featureGroups}" class="tablinks {isActive(featureGroup) ? \'-active\':\'\'}" onclick="{setFeatureGroup}">{getLabel(featureGroup)}</button></div>',
    '',
    '',
    function (opts) {
        this.featureGroups = [];
        ...
        this.on('mount', () => {
            this.opts.featureGroups.forEach((group) => {
                group.onFeatureClick.subscribe((f) => {
                    ...
                    this.setEntities(
                        f.properties?.entities
                            ?.filter((e) => e.visible !== false)
                            .filter((e) => this.getEntityLabel(e)?.length > 0)
                    );
        ...
        this.filterList = function (e) {
            ...
            if (filter) {
                this.filteredEntities = this.entities.filter((e) =>
                    this.getLabel(e).toLowerCase().includes(filter.toLowerCase())
                );
            } else {
                this.filteredEntities = undefined;
            }
                    ...
                    label = labels
                        .map((format) => {
                            ...
                            groups.forEach((group) => {
                                if (group.length > 1) {
                                    let value = entity[group[1]]
                                        ?.map((s) =>
                                            viewerJS.iiif.getValue(s, this.opts.locale, this.opts.defaultLocale)
                                        )
                                        .join(', ');
                                    if (value) {
...
riot.tag2(
    'slide_default',
    '<a class="swiper-link slider-{this.opts.stylename}__link" href="{this.opts.link}" target="{this.opts.link_target}" rel="noopener"><div class="swiper-heading slider-{this.opts.stylename}__header">{this.opts.label}</div><img class="swiper-image slider-{this.opts.stylename}__image" riot-src="{this.opts.image}" alt="{this.opts.alttext}"><p class="swiper-description slider-{this.opts.stylename}__description" ref="description"></p></a>',
    '',
    '',
    function (opts) {
        this.on('mount', () => {
...
riot.tag2(
    'slide_indexslider',
    '<a class="slider-{this.opts.stylename}__link-wrapper" href="{this.opts.link}"><div class="swiper-heading slider-mnha__header">{this.opts.label}</div><img class="slider-{this.opts.stylename}__image" loading="lazy" riot-src="{this.opts.image}"><div class="swiper-lazy-preloader"></div></a>',
    '',
    '',
    function (opts) {}
);
riot.tag2(
    'slide_stories',
    '<div class="slider-{this.opts.stylename}__image" riot-style="background-image: url({this.opts.image})"></div><a class="slider-{this.opts.stylename}__info-link" href="{this.opts.link}"><div class="slider-{this.opts.stylename}__info-symbol"><svg width="6" height="13" viewbox="0 0 6 13" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M4.664 1.21C4.664 2.134 4.092 2.728 3.168 2.728C2.354 2.728 1.936 2.134 1.936 1.474C1.936 0.506 2.706 0 3.454 0C4.136 0 4.664 0.506 4.664 1.21ZM5.258 11.528C4.664 12.1 3.586 12.584 2.42 12.716C1.386 12.496 0.748 11.792 0.748 10.78C0.748 10.362 0.836 9.658 1.1 8.58C1.276 7.81 1.452 6.534 1.452 5.852C1.452 5.588 1.43 5.302 1.408 5.236C1.144 5.17 0.726 5.104 0.198 5.104L0 4.488C0.572 4.07 1.716 3.718 2.398 3.718C3.542 3.718 4.202 4.312 4.202 5.566C4.202 6.248 4.026 7.194 3.828 8.118C3.542 9.328 3.432 10.12 3.432 10.472C3.432 10.802 3.454 11.022 3.542 11.154C3.96 11.066 4.4 10.868 4.928 10.56L5.258 11.528Z" fill="white"></path></svg></div><div class="slider-single-story__info-phrase">{this.opts.label}</div></a>',
    '',
    '',
    function (opts) {}
);
    ...
    function (opts) {
        ...
        this.on('mount', function () {
            ...
            if (this.opts.sourceelement) {
                ...
                pSource = fetch(this.opts.source).then((result) => result.json());
            }
            rxjs.from(pSource)
                .pipe(
        ...
        this.getImage = function (slide) {
            ...
            if (image == undefined) {
                ...
                let url =
                    viewerJS.iiif.getId(image.service) +
                    '/full/' +
                    this.getIIIFSize(this.style.imageWidth, this.style.imageHeight) +
                    '/0/default.jpg';
                return url;
            } else if (image['@id']) {
                return image['@id'];
