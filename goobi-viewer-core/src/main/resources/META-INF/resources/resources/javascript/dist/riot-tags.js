...
riot.tag2('thumbnails', '<div ref="thumb" class="thumbnails-image-wrapper {this.opts.index == index ? \'selected\' : \'\'} {getPageStatus(index)}" each="{canvas, index in thumbnails}"> <<<<<<< HEAD <a class="thumbnails-image-link" href="{getLink(canvas)}" onclick="{handleClickOnImage}"><img class="thumbnails-image" alt="{getObjectTitle() + \': \' + getValue(canvas.label)}" riot-src="{getImage(canvas)}" loading="lazy"> ======= <a class="thumbnails-image-link" href="{getLink(canvas)}" aria-label="{getAriaLabel(canvas, index)}" aria-current="{this.opts.index == index ? \'page\' : undefined}" tabindex="{needsKeyboardFocus(canvas) ? \'0\' : undefined}" role="{needsKeyboardFocus(canvas) ? \'link\' : undefined}" onclick="{handleClickOnImage}" onkeydown="{handleKeydownOnImage}"><img class="thumbnails-image" alt="" riot-src="{getImage(canvas)}" loading="lazy" data-viewer-thumbnail="thumbnail"> >>>>>>> 4307eee1db (show 404 and 403 placeholder images in archive view) <div class="thumbnails-image-overlay"><div class="thumbnails-label">{getValue(canvas.label)}</div></div></a></div>', '', '', function(opts) {
