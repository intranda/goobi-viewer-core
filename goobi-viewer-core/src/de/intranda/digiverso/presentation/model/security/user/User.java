...
public class User implements ILicensee, HttpSessionBindingListener {
	...
	/**
	 * Activation key for the user account. Value must be reset to null when the
	...
	/**
	 * Returns the name best suited for displaying (depending on which values are
	 * available).
	 *
	 * @return
	 */
	...
	/**
	 * If the display name is the e-mail address and the logged in user (!= this
	 * user) is not an superuser, obfuscate the address.
	 *
	 * @return
	 */
	public String getDisplayNameObfuscated() {
		...
		if (displayName.equals(email) && BeanUtils.getUserBean() != null && !BeanUtils.getUserBean().isAdmin()) {
			return new StringBuilder().append(Helper.getTranslation("user_anonymous", null)).append(" (").append(id)
					.append(')').toString();
		}
	...
	@Override
	public boolean hasLicense(String licenseName, String privilegeName, String pi)
		...
		for (License license : getLicenses()) {
			...
			if (license.isValid() && license.getLicenseType().getName().equals(licenseName)) {
				...
				if (license.getPrivileges().contains(privilegeName)
						|| license.getLicenseType().getPrivileges().contains(privilegeName)) {
					if (StringUtils.isEmpty(license.getConditions())) {
						...
						sbQuery.append(SolrConstants.PI).append(':').append(pi).append(" AND (")
								.append(license.getConditions()).append(')');
	...
	/**
	 * Checks whether the user can satisfy at least one of the given access
	 * conditions with a license that contains the given privilege name. If one of
	 * the conditions is OPENACCESS, true is always returned. Superusers always get
	 * access.
	 *
	 * @param conditionList
	 * @param privilegeName
	...
	public boolean isHasPrivilege(String privilege)
			throws PresentationException, IndexUnreachableException, DAOException {
		return canSatisfyAllAccessConditions(Collections.singletonMap(LicenseType.LICENSE_TYPE_CMS, null).keySet(),
				privilege, null);
	}

	/**
	 * Checks whether this user has the permission to set the representative image
	 * for the currently open record. TODO For some reason this method is called 8x
	 * in a row.
	 *
	 * @return
	 * @throws PresentationException
	...
	public boolean isMaySetRepresentativeImage() throws IndexUnreachableException, PresentationException, DAOException {
		...
		return isHasPrivilegeForCurrentRecord(LicenseType.LICENSE_TYPE_SET_REPRESENTATIVE_IMAGE,
				IPrivilegeHolder.PRIV_SET_REPRESENTATIVE_IMAGE, recordsForWhichUserMaySetRepresentativeImage);
	}

	/**
	 * Checks whether this user has the permission to delete all ocr-content of one
	 * page in crowdsourcing.
	 *
	 * @return
	 * @throws PresentationException
	 * @throws IndexUnreachableException
	 * @throws DAOException
	 */
	public boolean isMayDeleteCrowdsourcingFulltext()
			throws IndexUnreachableException, PresentationException, DAOException {
		return isHasPrivilegeForCurrentRecord(LicenseType.LICENSE_TYPE_DELETE_OCR_PAGE,
				IPrivilegeHolder.PRIV_DELETE_OCR_PAGE, recordsForWhichUserMayDeleteOcrPage);
	}
	...
	private boolean isHasPrivilegeForCurrentRecord(String licenseType, String privilegeName,
			Set<String> alreadyCheckedPiList) throws IndexUnreachableException, PresentationException, DAOException {
	 ...
	 * @throws DAOException
	 */
	private boolean isHasPrivilegeForRecord(String pi, String licenseType, String privilegeName,
			Set<String> alreadyCheckedPiList) throws PresentationException, IndexUnreachableException, DAOException {
