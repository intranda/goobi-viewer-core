...
public class ContentServerWrapperServlet extends HttpServlet implements Serializable {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (request.getParameterMap()
            ...
            try {
                String params = request.getRequestURI()
                        .replace(request.getContextPath() + "/content/", "");
                slashed = true;
                ...
                if ("image".equals(action) && pi != null) {
                    ...
                    String watermarkIdField = DataManager.getInstance()
                            .getConfiguration()
                            .getWatermarkIdField();
                    ...
                    SolrDocumentList docs = DataManager.getInstance()
                            .getSearchIndex()
                            .search(query, SolrSearchIndex.MAX_HITS, null, Arrays.asList(fieldNameFilter));
                    if (docs != null && docs.size() > 0) {
                        ...
                        if (fieldUrn != null) {
                            urlArgs.append("&watermarkText=")
                                    .append(fieldUrn);
                        }
                        String watermarkId = null;
                        if (doc.getFieldNames()
                                .contains(watermarkIdField)) {
                            ...
                            SolrDocumentList topDocList = DataManager.getInstance()
                                    .getSearchIndex()
                                    .search(new StringBuilder(SolrConstants.PI).append(':')
                                            .append(pi)
                                            .toString(), SolrSearchIndex.MAX_HITS, null, Collections.singletonList(watermarkIdField));
                            if (topDocList != null && topDocList.size() > 0) {
                                Object obj = topDocList.get(0)
                                        .getFieldValue(watermarkIdField);
                                if (obj instanceof String) {
                                    watermarkId = (String) topDocList.get(0)
                                            .getFieldValue(watermarkIdField);
                                } else if (obj instanceof ArrayList) {
                                    List<Object> objList = (List<Object>) obj;
                                    if (!objList.isEmpty()) {
                ...
                if (StringUtils.isNotEmpty(dataRepository)) {
                    mediaFilePath = new StringBuilder(dataRepository).append('/')
                    ...
                    mediaFilePath = new StringBuilder(pi).append('/')
                            .append(contentFileName)
                            .toString();
                }
                ...
                try {
                    ...
                    int maxImageWidth = DataManager.getInstance()
                            .getConfiguration()
                            .getViewerMaxImageWidth();
                    if (maxImageWidth > 0 && width > maxImageWidth) {
                        width = maxImageWidth;
                    }
                    urlArgs.append("&width=")
                            .append(width);
                } catch (NumberFormatException e) {
                }
                urlArgs.append("&rotate=")
                        .append(paramsSplit[2]);
                urlArgs.append("&compression=95");
            } catch (ArrayIndexOutOfBoundsException e) {
                logger.debug("Error processing request URL: {}", request.getRequestURL()
                        .toString());
        ...
        boolean isThumbnail = imageWidth <= DataManager.getInstance()
                .getConfiguration()
        ...
        if (access) {
            ...
            if (isThumbnail) {
                format = "jpg";
                urlArgs.append("&sourcepath=")
                        .append(ServletUtils.getServletPathWithHostAsUrlFromRequest(request))
                        .append("/resources/themes/")
                        .append(DataManager.getInstance()
                                .getConfiguration()
                                .getTheme())
                        .append("/images/thumbnail_access_denied.jpg");
            } else {
                format = "png";
                urlArgs.append("&ignoreWatermark=true&sourcepath=")
                        .append(ServletUtils.getServletPathWithHostAsUrlFromRequest(request))
                        .append("/resources/themes/")
                        .append(DataManager.getInstance()
                                .getConfiguration()
                                .getTheme())
                        .append("/images/access_denied.png");
            }
        }
        url.append(csType)
                .append("?action=")
        ...
        try {
            if (access) {
                ...
                if (DataManager.getInstance()
                        .getConfiguration()
                        .isUseExternalCS()
                        && ("image".equals(action) || "pdf".equals(action) || "dimensions".equals(action) || "version".equals(action))) {
                    ...
                    switch (action) {
                        case "image":
                            ...
                            if (!slashed && request.getParameterMap()
                                    .size() > 0) {
                                ...
                                if (requestMapCopy.get("height") != null) {
                                    ...
                                    int maxImageHeight = DataManager.getInstance()
                                            .getConfiguration()
                                ...
                                if (requestMapCopy.get("scale") != null) {
                                    ...
                                    int maxImageScale = DataManager.getInstance()
                                            .getConfiguration()
                        ...
                        case "audio": {
                            ...
                            File file;
                            if (StringUtils.isNotEmpty(dataRepository)) {
                                file = new File(DataManager.getInstance()
                                        .getConfiguration()
                                        .getDataRepositoriesHome() + mediaFilePath);
                            } else {
                                // Backwards compatibility with old indexes
                                file = new File(DataManager.getInstance()
                                        .getConfiguration()
                        ...
                        case "text": {
                            ...
                            if (StringUtils.isEmpty(dataRepository)) {
                                recordPath = DataManager.getInstance()
                                        .getConfiguration()
                                        .getViewerHome();
                            } else {
                                recordPath = new StringBuilder(DataManager.getInstance()
                                        .getConfiguration()
                                        .getDataRepositoriesHome()).append(dataRepository)
                                                .append("/")
                                                .toString();
                            }
                            ...
                            switch (format) {
                                case "alto":
                                    file = new File(new StringBuilder(recordPath).append(DataManager.getInstance()
                                            .getConfiguration()
                                            .getAltoFolder())
                                            .append('/')
                                            .append(pi)
                                            .toString(), contentFileName);
