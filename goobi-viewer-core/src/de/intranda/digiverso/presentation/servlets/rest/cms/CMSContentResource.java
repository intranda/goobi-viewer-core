 ...
 * 
 * getPageUrl(), getContentUrl() and getSidebarElementUrl() provide urls to cms-pages, content and sidebar-element respectively.
 * The other methods resolve these urls and return the appropriate html content. 
 * All urls are absolute urls including scheme information (http), as ui:include cannot resolve them otherwise (only file urls can be resolved with relative paths)
 */
@Path("/cms")
...
public class CMSContentResource {
	...
	public String getContentHtml(@PathParam("pageId") Long pageId, @PathParam("language") String language, @PathParam("contentId") String contentId) throws IOException, DAOException, ServletException {
	...
	private static String createResponseInThread(final TargetType target, final Long pageId, final String language, final String contentId , final long timeout/*ms*/) throws IOException, DAOException, ServletException {
		...
		Callable<String> job = new Callable<String>() {
			...
			try {
				...
				throw new DAOException("Timeout while accessing database");
			}
	    }
