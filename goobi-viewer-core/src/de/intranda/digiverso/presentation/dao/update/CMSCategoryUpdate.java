...
public class CMSCategoryUpdate implements IModelUpdate {
	 ...
	 * @throws DAOException
	 */
	public void persistData(IDAO dao) throws DAOException, SQLException {
		...
		if (this.entityMap.containsKey("content")) {
			dao.createNativeQuery(
					"ALTER TABLE cms_content_items DROP allowed_tags, DROP page_classification")
					.executeUpdate();
		}
	 ...
	 * @throws SQLException
	 */
	private Map<String, Map<String, List<Long>>> createEntityMap(IDAO dao) throws SQLException {
	 ...
	 * @param map
	 */
	private void linkToPages(List<CMSCategory> categories, Map<String, List<Long>> map, List<CMSPage> pages) {
		if (map != null) {
			for (String categoryName : map.keySet()) {
				...
				if (category == null) {
					...
					for (Long pageId : pageIds) {
						try {
							CMSPage page = pages.stream().filter(p -> pageId.equals(p.getId())).findFirst()
	...
	private void linkToMedia(List<CMSCategory> categories, Map<String, List<Long>> map, List<CMSMediaItem> mediaItems) {
		if (map != null) {
			for (String categoryName : map.keySet()) {
				CMSCategory category = categories.stream().filter(cat -> cat.getName().equalsIgnoreCase(categoryName))
						.findFirst().orElse(null);
				if (category == null) {
					...
					for (Long mediaId : mediaIds) {
						try {
							CMSMediaItem media = mediaItems.stream().filter(p -> mediaId.equals(p.getId())).findFirst()
							...
							logger.error("Error getting mediaItems for category {}. Failed to load mediaItem for id {}",
	 ...
	 * @throws DAOException
	 */
	private List<CMSCategory> synchronize(List<CMSCategory> categories, List<CMSCategory> existingCategories)
			throws DAOException {
