    ...
    public List<String> getImportedRecordsTrend(int days, int dataPoints) {
        ...
        try {
            QueryResponse resp = DataManager.getInstance().getSearchIndex().search(new StringBuilder(SolrConstants.PI).append(":*").append(
                    ...
                    Collections.singletonList(SolrConstants.DATECREATED), null);
    ...
    public List<String> getTopStructTypesByNumber() {
        ...
        try {
            QueryResponse resp = DataManager.getInstance().getSearchIndex().search(new StringBuilder(SolrConstants.PI).append(":*").append(
                    ...
                    Collections.singletonList(SolrConstants.DOCSTRCT), null);
    ...
    public Long getImportedFullTexts() {
        ...
        try {
            if (lastUpdateMap.get("getImportedFullTexts") == null || now - lastUpdateMap.get("getImportedFullTexts") >= DAY_MS) {
                ...
                // TODO filter query might not work for PAGE documents
                long pages = DataManager.getInstance().getSearchIndex().getHitCount(SolrConstants.DOCTYPE + ":" + SolrConstants.DocType.PAGE
                        .name() + " AND " + SolrConstants.FULLTEXTAVAILABLE + ":true" + SearchHelper.getAllSuffixes(false));
