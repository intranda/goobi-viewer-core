...
public final class SolrSearchIndex {
    ...
    /**
     * Returns a list of Lucene terms for the given field name.
     *
     * @param startsWith
     * @param filter
     * @return
     ...
     */
    @Deprecated
    public List<String> getTerms(String fieldName, String startsWith) throws PresentationException, IndexUnreachableException {
        List<String> ret = new ArrayList<>();

        SolrQuery solrQuery = new SolrQuery();
        solrQuery.setTermsLimit(MAX_HITS);
        solrQuery.setQueryType("/terms");
        solrQuery.setTerms(true);
        solrQuery.addTermsField(fieldName);

        try {
            QueryResponse resp = server.query(solrQuery);
            TermsResponse terms = resp.getTermsResponse();
            if (StringUtils.isNotEmpty(startsWith) && !"-".equals(startsWith)) {
                // With filtering
                if ("0-9".equals(startsWith)) {
                    // Numerical
                    Pattern p = Pattern.compile("[\\d]");
                    for (Term term : terms.getTerms(fieldName)) {
                        String termStart = term.getTerm();
                        if (termStart.length() > 1) {
                            termStart = term.getTerm().substring(0, 1);
                            // If the first char isn't alphanumeric, compare the second char
                            if (!StringUtils.isAlphanumeric(termStart)) {
                                termStart = term.getTerm().substring(1, 2);
                            }
                        }
                        Matcher m = p.matcher(termStart);
                        if (m.find()) {
                            ret.add(term.getTerm());
                        }
                    }
                } else {
                    // Alphabetical
                    for (Term term : terms.getTerms(fieldName)) {
                        if (StringUtils.startsWithIgnoreCase(term.getTerm(), startsWith) || term.getTerm().length() > 1 && !StringUtils
                                .isAlphanumeric(term.getTerm().substring(0, 1)) && StringUtils.startsWithIgnoreCase(term.getTerm().substring(1),
                                        startsWith)) {
                            ret.add(term.getTerm());
                        }
                    }
                ...
                // Without filtering
                for (Term term : terms.getTerms(fieldName)) {
                    if (term.getTerm().charAt(0) != 1) {
                        ret.add(term.getTerm());
                    }
                    // logger.debug(term.getTerm());
                }
            }
        } catch (SolrServerException e) {
            if (e.getMessage().startsWith("Server refused connection")) {
    ...
    /**
     * Generates a list of tags retrieved via a Solr term search.
     *
     *
     * @param fieldName {@link String} Lucene Feld, für den man den TagCloud machen will
     * @return {@link HashMap} key - TagName, value - count.
     * @throws IOException .
     */
    @Deprecated
    @SuppressWarnings("unchecked")
    public List<Tag> generateTagCloud(String fieldName) {
        // TODO Find a way to use a filter query in a term search.
        // TODO When fields contain reversed copies, the reversed values are returned too. Find a way to filter them out.
        ...
        try {
            ...
            // logger.debug("terms found: " + terms.getTerms(fieldName).size());
            for (Term term : terms.getTerms(fieldName)) {
                // collect items
                String text = term.getTerm();
                if (text.length() > 2 && text.charAt(0) != 1) {
                    // You must be at least this long to be added
                    // TODO Maybe use stopwords for exclusion, instead?
                    frequencyMap.put(text, term.getFrequency());
                    // logger.debug(text + ": " + term.getFrequency());
                    termlist.add(text);
                    // logger.debug("term: " + text + " - " + Integer.valueOf(text.charAt(0)));
                }
            }

            // Cutoff
            // TODO Muss konfigurierbar sein
            float topTermCutoff = 0.02F; // je grösser der index ist, desto größer wird dieser wert..
            ...
            if (e.getMessage().startsWith("IOException occured when talking to server") || e.getMessage().contains("Timeout")) {
                logger.warn("Solr communication timeout; Query: {}", solrQuery.getQuery());
            } else {
