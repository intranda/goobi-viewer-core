...
const { depsPathsCSS, depsPathsJS }  = require("./gulp/depsPaths");
function getTomcatDir() {
    const homedir = os.homedir();
    const rawdata = fs.readFileSync(path.join(homedir, ".config", "grunt_userconfig.json"), "utf-8");
    ...
    const viewer_config = XML.parse(fs.readFileSync(viewerCfgPath, "utf-8"));
    if (viewer_config.viewer.theme.specialName && viewer_config.viewer.theme.specialName.length) {
        return path.join(config.tomcat_dir, `goobi-viewer-theme-${viewer_config.viewer.theme.specialName}`);
    } else if (viewer_config.viewer.theme.deployedTargetFolder === "true") {
        return path.join(
            homedir,
            "git",
            "goobi-viewer",
        ...
        return path.join(config.tomcat_dir, `goobi-viewer-theme-${viewer_config.viewer.theme.mainTheme}`);
    }
}
