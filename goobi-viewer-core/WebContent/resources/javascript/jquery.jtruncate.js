			...
			if (a.length > h.length + h.minTrail) {
				...
				if (b != -1) {
					...
					if (a.length != 0 || a.length != -1) {
						var d = a.substring(b, a.length);
					} else {
						var d = a.substring(b, a.length - 1);
					}

					obj.html(c + '<span class="truncate_ellipsis">'
							+ h.ellipsisText + '</span>'
							+ '<span class="truncate_more">' + d + '</span>');
					obj.find('.truncate_more').css("display", "none");
					obj.append('<span class="clearboth">'
							+ '<span class="truncate_more_link">' + h.moreText
							+ '</span>' + '</span>');
